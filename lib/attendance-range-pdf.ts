...
function drawHeader(ctx:CanvasRenderingContext2D,o:AttendanceRangePdfOptions,report:AttendanceRangePdfClass,accent:string,logo:HTMLImageElement|null,pageIndex:number,pageCount:number){
  roundedRect(ctx,34,28,PRINT_WIDTH-68,138,22,"#fff","#d9e6e3");ctx.fillStyle=accent;ctx.fillRect(34,28,PRINT_WIDTH-68,12);
  roundedRect(ctx,48,48,122,102,18,"#fff","#d6e3df");if(logo)drawImageContain(ctx,logo,54,54,110,90,2);
  drawFittedText(ctx,o.portalName,188,70,{size:23,min:18,weight:900,color:"#60777f",align:"left",maxWidth:520});drawFittedText(ctx,"تقرير الحضور والانضباط للفترة",188,112,{size:34,min:27,weight:900,color:"#153b49",align:"left",maxWidth:760});drawFittedText(ctx,`${o.from} إلى ${o.to}`,188,143,{size:17,min:14,weight:800,color:accent,align:"left",maxWidth:500});
  roundedRect(ctx,PRINT_WIDTH-400,52,334,94,18,accent);drawFixedText(ctx,"سجل المتابعة للفترة",PRINT_WIDTH-92,82,{size:25,weight:900,color:"#fff",maxWidth:275});drawFixedText(ctx,`${report.className} • ${pageIndex+1}/${pageCount}`,PRINT_WIDTH-92,120,{size:16,weight:850,color:"#e3f5f2",maxWidth:275});
  const meta=[["المعلم",o.teacherName],["المادة",o.subject],["الفصل",report.className],["الفترة",`${o.from} — ${o.to}`],["أيام التحضير",report.days.length]];const gap=10,margin=34,top=180,boxW=(PRINT_WIDTH-margin*2-gap*4)/5;
  meta.forEach(([label,value],i)=>{const x=PRINT_WIDTH-margin-boxW-i*(boxW+gap);roundedRect(ctx,x,top,boxW,72,13,"#f7faf9","#dce7e4");drawFixedText(ctx,label,x+boxW-14,top+22,{size:14,weight:850,color:"#7a9095",maxWidth:boxW-28});drawFittedText(ctx,value,x+boxW-14,top+50,{size:19,min:15,weight:900,color:"#21464c",maxWidth:boxW-28});});
  ...
  summary.forEach(([label,value],i)=>{const x=PRINT_WIDTH-34-sw-i*(sw+gap);const good=i===1;roundedRect(ctx,x,266,sw,68,13,good?"#e6f5ed":"#f7faf9","#dfe8e6");drawFixedText(ctx,value,x+sw/2,288,{size:23,weight:900,color:good?"#216c4c":"#244b51",align:"center",maxWidth:sw-20});drawFixedText(ctx,label,x+sw/2,316,{size:13,weight:850,color:"#667d84",align:"center"});});
}
function drawTable(ctx:CanvasRenderingContext2D,rows:AttendanceRangePdfRow[],accent:string){
  const x=34,top=352,w=PRINT_WIDTH-68,bottom=PRINT_HEIGHT-104,headerH=58;const rowH=Math.floor((bottom-top-headerH)/Math.max(rows.length,17));
  const widths=[62,330,92,235,220,220,220,115];const labels=["م","اسم الطالب","حضور","تواريخ الغياب","تواريخ التأخير","تواريخ الاستئذان","تواريخ الهروب","النسبة"];
  ...
  labels.forEach((label,i)=>{const ww=widths[i];drawFixedText(ctx,label,cursor-ww/2,top+headerH/2,{size:15,weight:900,color:"#fff",align:"center",maxWidth:ww-10});cursor-=ww;printLine(ctx,cursor,top,cursor,bottom,"rgba(255,255,255,.25)",1);});
  rows.forEach((row,index)=>{const y=top+headerH+index*rowH;ctx.fillStyle=index%2?"#f6faf9":"#fff";ctx.fillRect(x,y,w,rowH);printLine(ctx,x,y+rowH,x+w,y+rowH,"#e1e9e7");let r=x+w;const values=[row.number,row.name,row.present,dates(row.absentDates),dates(row.lateDates),dates(row.excusedDates),dates(row.escapedDates),`${row.attendanceRate}%`];values.forEach((value,i)=>{const ww=widths[i];const isName=i===1;const isRate=i===7;if(isName)drawFixedText(ctx,value,r-14,y+rowH/2,{size:STUDENT_NAME_FONT_SIZE,weight:850,maxWidth:ww-28});else drawFixedText(ctx,value,r-ww/2,y+rowH/2,{size:i>=3&&i<=6?13.5:15.5,weight:isRate?900:800,color:isRate?accent:undefined,align:"center",maxWidth:ww-12});r-=ww;printLine(ctx,r,y,r,y+rowH);});});ctx.restore();
}
function drawFooter(ctx:CanvasRenderingContext2D,o:AttendanceRangePdfOptions,report:AttendanceRangePdfClass,accent:string,pageIndex:number,pageCount:number){const y=PRINT_HEIGHT-64;printLine(ctx,34,y-22,PRINT_WIDTH-34,y-22,"#c8d7d3",1.4);drawFixedText(ctx,"اعتماد المعلم: __________________________",PRINT_WIDTH-34,y,{size:14,weight:800,color:"#60777f",maxWidth:470});drawFixedText(ctx,`صفحة ${pageIndex+1} من ${pageCount}`,PRINT_WIDTH/2,y,{size:13.5,weight:900,color:"#60777f",align:"center",maxWidth:220});drawFixedText(ctx,`${report.className} • ${o.from} إلى ${o.to}`,34,y,{size:13.5,weight:900,color:accent,align:"left",maxWidth:520});}
