...
async function loadPortalLogo(){
  return new Promise<HTMLImageElement|null>(resolve=>{
    const image=new Image();image.onload=()=>resolve(image);image.onerror=()=>resolve(null);image.src="/icons/lahooni-identity-320.jpg";
  });
}
function drawHeader(ctx:CanvasRenderingContext2D,options:AttendancePdfDocumentOptions,report:AttendancePdfClass,accent:string,classIndex:number,logo:HTMLImageElement|null,pageIndex:number,pageCount:number){
  ...
  rounded(ctx,50,42,100,100,22,"#ffffff","#d6e3df");
  if(logo)ctx.drawImage(logo,58,50,84,84);
  txt(ctx,options.portalName,170,63,{size:19,weight:900,color:"#5f787d",align:"left",maxWidth:500});
  txt(ctx,"تقرير الحضور والانضباط اليومي",170,108,{size:30,min:22,weight:900,color:"#173d45",align:"left",maxWidth:680});
  rounded(ctx,170,132,240,24,12,"#f3f7f6");
  txt(ctx,`الفصل ${classIndex+1} من ${options.classes.length}${pageCount>1?` • صفحة ${pageIndex+1}/${pageCount}`:""}`,290,144,{size:11.5,weight:900,color:accent,align:"center",maxWidth:220});
  ...
  meta.forEach(([label,value],index)=>{const x=WIDTH-margin-boxW-index*(boxW+gap);rounded(ctx,x,top,boxW,66,13,"#f8fbfa","#dce7e4");txt(ctx,label,x+boxW-14,top+20,{size:11,weight:900,color:"#849598",maxWidth:boxW-28});txt(ctx,value,x+boxW-14,top+45,{size:16,min:10.5,weight:900,color:"#21464c",maxWidth:boxW-28});});
  ...
  const sw=(WIDTH-68-gap*5)/6;summary.forEach(([label,value,bg,fg],index)=>{const x=WIDTH-34-sw-index*(sw+gap);rounded(ctx,x,252,sw,62,13,bg,"#dfe8e6");txt(ctx,value,x+sw/2,272,{size:20,min:13,weight:900,color:fg,align:"center",maxWidth:sw-18});txt(ctx,label,x+sw/2,296,{size:11,weight:900,color:fg,align:"center"});});
}
function drawTable(ctx:CanvasRenderingContext2D,rows:AttendancePdfRow[],accent:string){
  const x=34,top=332,w=WIDTH-68,bottom=HEIGHT-100,headerH=48;
  const rowH=Math.floor((bottom-top-headerH)/Math.max(rows.length,18));
  const compact=rows.length>30;const numW=74,statusW=220,noteW=350,nameW=w-numW-statusW-noteW;
  ...
  cols.forEach(col=>{const center=cursor-col.w/2;txt(ctx,col.label,center,top+headerH/2,{size:14,weight:900,color:"#fff",align:"center",maxWidth:col.w-12});cursor-=col.w;line(ctx,cursor,top,cursor,bottom,"rgba(255,255,255,.26)",1);});
  rows.forEach((row,index)=>{const y=top+headerH+index*rowH;ctx.fillStyle=index%2?"#f8fbfa":"#fff";ctx.fillRect(x,y,w,rowH);line(ctx,x,y+rowH,x+w,y+rowH,"#e4ecea");let r=x+w;
    txt(ctx,row.number,r-numW/2,y+rowH/2,{size:compact?10.5:13,weight:900,align:"center"});r-=numW;line(ctx,r,y,r,y+rowH);
    txt(ctx,row.name,r-14,y+rowH/2,{size:compact?11.5:14,min:9.5,weight:900,maxWidth:nameW-28});r-=nameW;line(ctx,r,y,r,y+rowH);
    const style=statusColors(row.status);const pillH=Math.max(12,Math.min(28,rowH-6));rounded(ctx,r-statusW+34,y+(rowH-pillH)/2,statusW-68,pillH,pillH/2,style.bg);txt(ctx,row.status,r-statusW/2,y+rowH/2,{size:compact?10:12,min:8.5,weight:900,color:style.fg,align:"center",maxWidth:statusW-80});r-=statusW;line(ctx,r,y,r,y+rowH);
  });ctx.restore();
}
function drawFooter(ctx:CanvasRenderingContext2D,options:AttendancePdfDocumentOptions,report:AttendancePdfClass,accent:string,pageIndex:number,pageCount:number){
  ...
  txt(ctx,"اعتماد المعلم: __________________________",WIDTH-34,y,{size:12,weight:800,color:"#647b80",maxWidth:450});
  txt(ctx,`صفحة ${pageIndex+1} من ${pageCount}`,WIDTH/2,y,{size:11.5,weight:900,color:"#647b80",align:"center",maxWidth:220});
...
function render(options:AttendancePdfDocumentOptions,report:AttendancePdfClass,classIndex:number,rows:AttendancePdfRow[],logo:HTMLImageElement|null,pageIndex:number,pageCount:number){
  const {canvas,ctx}=canvasPage();const accent=report.accentColor||CLASS_ACCENTS[classIndex%CLASS_ACCENTS.length]||DEFAULT_ACCENT;drawHeader(ctx,options,report,accent,classIndex,logo,pageIndex,pageCount);drawTable(ctx,rows,accent);drawFooter(ctx,options,report,accent,pageIndex,pageCount);return canvas;
}
