...
function drawHeader(ctx:CanvasRenderingContext2D,options:AttendancePdfDocumentOptions,report:AttendancePdfClass,accent:string,classIndex:number,logo:HTMLImageElement|null,pageIndex:number,pageCount:number){
  roundedRect(ctx,34,28,PRINT_WIDTH-68,142,22,"#ffffff","#d8e6e2");
  ctx.fillStyle=accent;ctx.fillRect(34,28,PRINT_WIDTH-68,13);
  roundedRect(ctx,48,48,126,106,18,"#ffffff","#d6e3df"); if(logo) drawImageContain(ctx,logo,54,54,114,94,2);
  drawFittedText(ctx,options.portalName,192,70,{size:24,weight:900,color:"#5b747b",align:"left",maxWidth:530});
  drawFittedText(ctx,"سجل المتابعة الأكاديمي",192,113,{size:36,weight:900,color:"#153b49",align:"left",maxWidth:740});
  drawFittedText(ctx,"تقرير حضور وانضباط صادر من بوابة تعليمية ذكية",192,146,{size:18,weight:750,color:"#6c8187",align:"left",maxWidth:770});
  roundedRect(ctx,PRINT_WIDTH-394,52,328,96,18,accent);
  drawFixedText(ctx,"الحضور اليومي",PRINT_WIDTH-92,83,{size:28,weight:900,color:"#fff",maxWidth:270});
  drawFixedText(ctx,`${report.className} • ${pageIndex+1}/${pageCount}`,PRINT_WIDTH-92,122,{size:18,weight:850,color:"#dff4f1",maxWidth:270});
  ...
  const gap=10,margin=34,top=184,boxW=(PRINT_WIDTH-margin*2-gap*4)/5;
  meta.forEach(([label,value],index)=>{
    const x=PRINT_WIDTH-margin-boxW-index*(boxW+gap);
    roundedRect(ctx,x,top,boxW,76,13,"#f7faf9","#dce7e4");
    drawFixedText(ctx,label,x+boxW-14,top+23,{size:15,weight:850,color:"#7a9095",maxWidth:boxW-28});
    drawFittedText(ctx,value,x+boxW-14,top+53,{size:20,weight:900,color:"#21464c",maxWidth:boxW-28});
  });
  ...
  summary.forEach(([label,value,bg,fg],index)=>{const x=PRINT_WIDTH-34-sw-index*(sw+gap);roundedRect(ctx,x,276,sw,70,13,bg,"#dfe8e6");drawFixedText(ctx,value,x+sw/2,298,{size:25,weight:900,color:fg,align:"center",maxWidth:sw-18});drawFixedText(ctx,label,x+sw/2,327,{size:14,weight:850,color:fg,align:"center"});});
}
function drawTable(ctx:CanvasRenderingContext2D,rows:AttendancePdfRow[],accent:string){
  const x=34,top=366,w=PRINT_WIDTH-68,bottom=PRINT_HEIGHT-108,headerH=60;
  const rowH=Math.floor((bottom-top-headerH)/Math.max(rows.length,16));
  const numW=72,statusW=230,noteW=310,nameW=w-numW-statusW-noteW;
  ...
  cols.forEach(col=>{const center=cursor-col.w/2;drawFixedText(ctx,col.label,center,top+headerH/2,{size:18,weight:900,color:"#fff",align:"center",maxWidth:col.w-12});cursor-=col.w;printLine(ctx,cursor,top,cursor,bottom,"rgba(255,255,255,.28)",1);});
  rows.forEach((row,index)=>{
    ...
    drawFixedText(ctx,row.number,r-numW/2,y+rowH/2,{size:18,weight:850,align:"center"});r-=numW;printLine(ctx,r,y,r,y+rowH);
    ...
    const style=statusColors(row.status);const pillH=Math.max(26,Math.min(36,rowH-8));roundedRect(ctx,r-statusW+34,y+(rowH-pillH)/2,statusW-68,pillH,pillH/2,style.bg);drawFixedText(ctx,row.status,r-statusW/2,y+rowH/2,{size:17,weight:900,color:style.fg,align:"center",maxWidth:statusW-80});r-=statusW;printLine(ctx,r,y,r,y+rowH);
  });
  ctx.restore();
}
function drawFooter(ctx:CanvasRenderingContext2D,options:AttendancePdfDocumentOptions,report:AttendancePdfClass,accent:string,pageIndex:number,pageCount:number){const y=PRINT_HEIGHT-64;printLine(ctx,34,y-22,PRINT_WIDTH-34,y-22,"#c8d7d3",1.4);drawFixedText(ctx,"اعتماد المعلم: __________________________",PRINT_WIDTH-34,y,{size:15,weight:800,color:"#60777f",maxWidth:470});drawFixedText(ctx,`صفحة ${pageIndex+1} من ${pageCount}`,PRINT_WIDTH/2,y,{size:15,weight:900,color:"#60777f",align:"center",maxWidth:220});drawFixedText(ctx,`${options.portalName} • ${report.className}`,34,y,{size:15,weight:900,color:accent,align:"left",maxWidth:500});}
