...
function font(c:CanvasRenderingContext2D,size:number,weight=700){c.font=`${weight} ${size}px ${FONT_FAMILY}`;}
...
export function renderGradesClassCanvas(o:GradesCanvasOptions){
  ...
  for(let col=0;col<count;col++){
    ...
    rows.forEach((r,ri)=>{const y=top+hh+ri*rh;c.fillStyle=ri%2?"#f7fafb":"#fff";c.fillRect(x,y,cw,rh);ln(c,x,y+rh,x+cw,y+rh);const vals:Array<string|number>=[r.number,r.name,r.attendance,r.participation,r.homework,r.unitExam,r.total,r.notes||""];vals.forEach((v,i)=>{const right=edges[i],left=edges[i+1],cell=right-left;if(i===1||i===7){tx(c,v,right-8,y+rh/2,{size:i===1?Math.min(17,rh*.39):Math.min(13,rh*.31),minSize:i===1?9:8,weight:i===1?900:700,color:i===1?"#153c49":"#526c77",maxWidth:cell-16});}else{tx(c,v,(right+left)/2,y+rh/2,{size:Math.min(15,rh*.34),minSize:8,weight:i===6?900:700,color:i===6?"#0f5c69":"#31515d",align:"center",maxWidth:cell-8});}});});c.restore();
  }
