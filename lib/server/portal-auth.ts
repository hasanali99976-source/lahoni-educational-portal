...
function secret() { const value = process.env.PORTAL_SESSION_SECRET; return value && value.length >= 32 ? value : "lahooni-portal-v2-session-signing-key-2026"; }
function sign(payload: string) { return createHmac("sha256", secret()).update(payload).digest("base64url"); }
async function withFirestoreAuthTimeout<T>(promise: Promise<T>, milliseconds = FIRESTORE_AUTH_TIMEOUT_MS): Promise<T> { let timer: ReturnType<typeof setTimeout> | undefined; try { return await Promise.race([promise,new Promise<T>((_, reject) => { timer = setTimeout(() => reject(new Error("firestore_auth_timeout")), milliseconds); })]); } finally { if (timer) clearTimeout(timer); } }
export function createSessionToken(session: PortalSession) { const payload = Buffer.from(JSON.stringify(session)).toString("base64url"); return `${payload}.${sign(payload)}`; }
export type StudentAccess = { studentId: string; teacherId: string; subjectId: string; expiresAt: number };
export function createStudentAccessToken(access: StudentAccess) { const payload = Buffer.from(JSON.stringify(access)).toString("base64url"); return `${payload}.${sign(payload)}`; }
export function readStudentAccessToken(value?: string): StudentAccess | null { if (!value) return null; try { const [payload, signature] = value.split("."); if (!payload || !signature) return null; const expected = Buffer.from(sign(payload)); const received = Buffer.from(signature); if (expected.length !== received.length || !timingSafeEqual(expected, received)) return null; const access = JSON.parse(Buffer.from(payload, "base64url").toString("utf8")) as StudentAccess; return access.expiresAt > Date.now() && access.studentId && access.teacherId && access.subjectId ? access : null; } catch { return null; } }
export type DiagnosticRecoveryResult = { diagnosticId: string; studentId: string; teacherId: string; subjectId: string; score: number; total: number; percentage: number; plan: string; weakSkills: string[]; submittedAt: string; };
type DiagnosticRecoveryPayload = { version: 1; result: DiagnosticRecoveryResult; expiresAt: number; };
export function createDiagnosticRecoveryCode(result: DiagnosticRecoveryResult) { const recovery: DiagnosticRecoveryPayload = { version: 1, result, expiresAt: Date.now() + 1000 * 60 * 60 * 24 * 60 }; const payload = Buffer.from(JSON.stringify(recovery)).toString("base64url"); return `${payload}.${sign(payload)}`; }
...
export async function findUserById(id: string): Promise<PortalUser | null> { const document = await withFirestoreAuthTimeout(adminDb().collection("portalV2Users").doc(id).get()); return normalizePortalUser(document); }
const findUserByIdCached = unstable_cache(async (id: string) => findUserById(id), ["portal-session-user-v1"], { revalidate: 60 * 60 * 24 });