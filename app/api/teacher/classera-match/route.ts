...
function normalizeArabic(value:unknown){return String(value??"").normalize("NFKC").replace(/[أإآٱ]/g,"ا").replace(/ى/g,"ي").replace(/ة/g,"ه").replace(/[ًٌٍَُِّْـ]/g,"").replace(/[^\p{L}\p{N}]+/gu," ").replace(/\s+/g," ").trim().toLowerCase()}
...
export async function POST(request:Request){
  ...
  try{
    ...
    const normalizedRoster=roster.map(item=>({...item,normalized:normalizeArabic(item.name)}));
    const counts:Record<string,number>={};
    const perFile:Array<{name:string;matched:number}>=[];
    for(const file of files){
      ...
      let matched=0;
      for(const student of normalizedRoster){if(student.normalized.length>=5&&normalizedText.includes(` ${student.normalized} `)){counts[student.code]=(counts[student.code]||0)+1;matched++}}
      perFile.push({name:clean(file.name,120),matched});
    }
    ...
    return NextResponse.json({ok:true,totalFiles:files.length,rows,files:perFile},{headers:{"Cache-Control":"no-store"}});
