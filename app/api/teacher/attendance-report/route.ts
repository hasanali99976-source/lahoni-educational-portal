...
async function readReport(key: string, query: FirebaseFirestore.Query) {
  ...
  const promise = query.get().then(snapshot => {
    const rows = snapshot.docs.map(document => ({ id: document.id, ...(document.data() as Record<string, unknown>) }));
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, attendance }, { headers: { "Cache-Control": "private, max-age=60, stale-while-revalidate=240" } });
  } catch (error) {
