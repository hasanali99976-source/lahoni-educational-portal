...
export async function POST(request: Request) {
  try {
    ...
    if (!username || !password) {
      return NextResponse.json({ ok: false, message: "أدخل اسم المستخدم وكلمة المرور" }, { status: 400 });
    }

    const user = await findUserByUsername(username);
    if (!user || !user.active || !user.updatedAt || !verifyPassword(password, user.passwordHash)) {
