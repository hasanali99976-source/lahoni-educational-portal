...
const css = `
@import url('https://fonts.googleapis.com/css2?family=Alexandria:wght@400;500;600;700;800&family=Noto+Kufi+Arabic:wght@500;600;700;800;900&display=swap');
#lahooni-home{min-height:100dvh;direction:rtl;position:relative;overflow-x:hidden;background:#071b29;color:#fff;font-family:"Alexandria","Noto Kufi Arabic","Segoe UI",Arial,sans-serif}
...
.brand{display:flex;align-items:center;gap:12px}.brand img{width:54px;height:54px;border-radius:18px;object-fit:cover;border:2px solid #edc56f;box-shadow:0 0 0 4px rgba(237,197,111,.08),0 12px 26px rgba(0,0,0,.26)}.brand strong{display:block;font-family:"Noto Kufi Arabic","Alexandria",sans-serif;font-size:16px;font-weight:800;letter-spacing:-.45px}.brand span{display:block;margin-top:2px;font-size:9px;color:#d2e0e4;font-weight:500}
...
.scene-tab{position:relative;overflow:hidden;text-decoration:none;padding:10px 15px;border-radius:17px;color:#dce8ec;font-size:10px;font-weight:600;letter-spacing:-.15px;border:1px solid transparent;transition:.3s cubic-bezier(.2,.8,.2,1);transform-style:preserve-3d}.scene-tab:before{content:"";position:absolute;inset:-1px auto -1px -70%;width:52%;background:linear-gradient(110deg,transparent,rgba(255,255,255,.45),transparent);transform:skewX(-16deg);transition:.45s}.scene-tab:hover{transform:translateY(-3px) translateZ(18px);color:#fff;background:rgba(255,255,255,.09);border-color:rgba(255,255,255,.14);box-shadow:0 12px 24px rgba(0,0,0,.18)}.scene-tab:hover:before{left:125%}.scene-tab.active{background:linear-gradient(145deg,#f5d584,#c98e33);color:#112f3f;box-shadow:0 10px 25px rgba(202,143,49,.30),inset 0 1px rgba(255,255,255,.65)}.scene-tab.active:after{content:"";position:absolute;left:18%;right:18%;bottom:4px;height:2px;border-radius:999px;background:rgba(16,47,64,.45);animation:pulseLine 2s ease-in-out infinite}@keyframes pulseLine{50%{opacity:.35;transform:scaleX(.7)}}
.hero{position:relative;z-index:4;text-align:center;padding:23px 18px 13px}.hero .kicker{display:inline-flex;align-items:center;gap:7px;padding:7px 13px;border-radius:999px;background:rgba(4,31,47,.42);border:1px solid rgba(240,202,111,.26);color:#f0d48e;font-size:10px;font-weight:600;backdrop-filter:blur(10px);box-shadow:0 10px 24px rgba(0,0,0,.12)}.hero .kicker:before{content:"";width:7px;height:7px;border-radius:50%;background:#f2c964;box-shadow:0 0 0 5px rgba(242,201,100,.12),0 0 16px rgba(242,201,100,.8);animation:beacon 2.2s ease-in-out infinite}@keyframes beacon{50%{box-shadow:0 0 0 8px rgba(242,201,100,0),0 0 22px rgba(242,201,100,.95)}}
.hero h1{margin:9px 0 0;font-family:"Noto Kufi Arabic","Alexandria",sans-serif;font-size:clamp(40px,5vw,68px);line-height:1.35;font-weight:900;letter-spacing:-2.2px;background:linear-gradient(180deg,#fffdf5 0%,#f8df9a 42%,#e5ad4d 82%,#c98627 100%);-webkit-background-clip:text;background-clip:text;color:transparent;filter:drop-shadow(0 7px 0 rgba(83,49,12,.28)) drop-shadow(0 15px 28px rgba(0,0,0,.34));text-wrap:balance}.hero p{margin:8px auto 0;max-width:790px;font-size:12px;line-height:1.95;color:#eef5f6;font-weight:500}.hero .rule{width:180px;height:3px;margin:12px auto 0;border-radius:999px;background:linear-gradient(90deg,transparent,#f0c86b,transparent);box-shadow:0 0 28px rgba(240,200,107,.52)}
...
@media(max-width:900px){.stage{width:98vw;padding:7px 0 10px;display:block}.stage:before{inset:4px;border-radius:26px}.topline{margin:0 7px;padding:9px 10px;border-radius:21px}.scene-tabs{display:none}.brand img{width:46px;height:46px;border-radius:15px}.brand strong{font-size:12px}.brand span{font-size:8px}.hero{padding:22px 9px 13px}.hero h1{font-size:31px;letter-spacing:-1px;line-height:1.45}.hero p{font-size:9.4px}.portal-zone{width:94%}.cards{grid-template-columns:1fr;direction:rtl;gap:10px}.portal-card{min-height:158px;display:grid;grid-template-columns:76px 1fr;grid-template-rows:auto auto 40px;text-align:right;padding:11px}.portal-icon{grid-row:1/3;width:65px;height:65px;margin:0;border-radius:20px;font-size:27px}.portal-card h2{font-size:15px;margin:0}.portal-card p{font-size:8.7px}.enter{grid-column:1/3;width:100%;height:40px}.scene-icons{width:94%;grid-template-columns:1fr 1fr}.scene-feature{justify-content:flex-start}.support-feature{grid-column:1/-1}.signature{margin-top:9px;padding-bottom:9px}}
`;
    ...
    <div className="stage" id="home">
      ...
      <div>
        <section className="scene-icons" id="features" aria-label="مزايا المنصة">
          ...
          <div className="scene-feature support-feature" id="support"><i>☎</i><span><b>الدعم الفني</b>للمساعدة والاستفسارات<a className="support-link" href="tel:0590701007">0590701007</a></span></div>
        </section>
