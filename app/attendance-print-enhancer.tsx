...
function printAttendance() {
  ...
  const pageHtml = pages.map((pageRows, pageIndex) => `
    <section class="print-page">
      ...
      <table><thead><tr><th>م</th><th>اسم الطالب</th><th>الحالة</th><th>ملاحظات</th></tr></thead><tbody>
        ${pageRows.map(row => `<tr><td>${escapeHtml(row.number)}</td><td>${escapeHtml(row.name)}</td><td class="status">${escapeHtml(row.status)}</td><td></td></tr>`).join("")}
      </tbody></table>
