...
export default function StudentDiagnostics({ accessToken }: { accessToken: string }) {
  ...
  async function submit() {
    ...
    setSubmitting(true);
    try {
      const response = await fetch("/api/student/diagnostics", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ diagnosticId: active.id, answers }) });
      const data = await response.json();
      if (!response.ok) return setMessage(data.message || "تعذر تسليم الاختبار.");
      setResultView({ title: active.title, result: data.result });
      setActive(null); setAnswers({}); setTab("ai");
      setMessage("تم تسليم الاختبار وتحديث تحليلك الذكي وخطتك العلاجية.");
      await load(false);
