...
type UnitRecord={total?:number;attendance?:number;participation?:number;homework?:number;unitExam?:number;exam1?:number;exam2?:number;notes?:string;updatedAt?:string;maximumTotal?:number;percentage?:number};
...
export default function StudentPage(){
 ...
 const unitRows=useMemo(()=>units.map(([key,label])=>{const r=student?.units?.[key]||{},attendance=Number(r.attendance||0),participation=Number(r.participation||0),homework=Number(r.homework||0),unitExam=Number(r.unitExam??r.exam1??r.exam2??0),total=Number(r.total??attendance+participation+homework+unitExam),isRecorded=Boolean(r.updatedAt)||r.total!==undefined||r.attendance!==undefined||r.participation!==undefined||r.homework!==undefined||r.unitExam!==undefined||r.exam1!==undefined||r.exam2!==undefined;return{key,label,attendance,participation,homework,unitExam,total,notes:r.notes||"",isRecorded}}),[student]);
 const research=Number(student?.researchScore??student?.research??0),unitsTotal=unitRows.reduce((s,u)=>s+u.total,0),finalTotal=Math.min(100,unitsTotal+research),motivational=encouragement(finalTotal),recordedUnits=unitRows.filter(u=>u.isRecorded),averageUnits=recordedUnits.length?Math.round(recordedUnits.reduce((s,u)=>s+u.total,0)/recordedUnits.length*10)/10:0,remaining=Math.max(0,100-finalTotal);
 ...
 <section className="student-mini-stats"><article><span>الحضور</span><strong>{pct(attendanceRate)}</strong></article><article><span>متوسط درجات الوحدات المرصودة</span><strong>{ar(averageUnits)}/١٩</strong><small>محسوب من {ar(recordedUnits.length)} وحدات</small></article><article><span>البحث</span><strong>{ar(research)}/٥</strong></article><article><span>الوحدات المرصودة</span><strong>{ar(recordedUnits.length)}/٥</strong></article></section>
 <section className="student-units-table"><div className="student-section-title"><div><h2>درجات الوحدات</h2><p>تحليل واضح لكل عنصر من عناصر الدرجة.</p></div></div><div className="student-table-scroll"><table><thead><tr><th>الوحدة</th><th>الحضور</th><th>المشاركة</th><th>الواجبات</th><th>الاختبار</th><th>المجموع</th><th>التقدير</th></tr></thead><tbody>{unitRows.map(u=><tr key={u.key}><td><b>{u.label}</b></td><td>{ar(u.attendance)}/٣</td><td>{ar(u.participation)}/٤</td><td>{ar(u.homework)}/٢</td><td>{ar(u.unitExam)}/١٠</td><td><strong>{ar(u.total)}/١٩</strong></td><td><span className={`grade-pill ${u.total>=15?"good":u.isRecorded?"warn":"empty"}`}>{u.isRecorded?gradeLabel(u.total):"لم يُرصد"}</span></td></tr>)}</tbody></table></div></section>
