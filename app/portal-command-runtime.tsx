...
const COMMANDS: Array<{ pattern: RegExp; command: string; icon: string }> = [
...
function currentStudentPanel(root: HTMLElement) {
...
function printableClone(source: HTMLElement | null) {
...
function openStudentPrintReport() {
  ...
  const header = printableClone(root.querySelector<HTMLElement>(".student-clean-head"));
  const panel = printableClone(currentStudentPanel(root));
...
body{padding:20px}.toolbar{position:sticky;top:0;z-index:5;display:flex;justify-content:center;gap:10px;padding:10px;margin:-20px -20px 18px;background:#0b736b}.toolbar button{border:0;border-radius:10px;padding:11px 18px;font:inherit;font-weight:800;cursor:pointer}
.report{width:min(900px,100%);margin:auto;background:#fff;padding:20px;border-radius:18px;box-shadow:0 12px 34px rgba(17,57,66,.12)}.report-brand{text-align:center;border-bottom:2px solid #0b736b;padding-bottom:10px;margin-bottom:14px}.report-brand strong{display:block;font-size:20px;color:#0b736b}.report-brand span{font-size:12px;color:#667b84}
.student-clean-head{display:block!important;background:#fff!important;color:#111!important;border:2px solid #0b736b!important;border-radius:14px!important;padding:16px!important;margin-bottom:14px!important}.student-clean-head h1{margin:6px 0!important;font-size:24px!important}.student-clean-head p,.student-clean-head span{color:#333!important}
.student-tab-panel,.student-main-summary,.student-attendance-summary,.student-home-grid article,.student-mini-stats article,.student-units-table,.student-goal-panel,.student-ai-hub,.student-diagnostics{background:#fff!important;color:#111!important;border:1px solid #c8d5d8!important;border-radius:12px!important;box-shadow:none!important}.student-tab-panel{display:block!important;margin:0!important;padding:0!important}.student-main-summary,.student-attendance-summary,.student-units-table,.student-goal-panel,.student-ai-hub,.student-diagnostics{padding:14px!important;margin-bottom:12px!important}
.student-mini-stats,.student-home-grid,.student-ai-grid,.attendance-discipline-grid,.goal-numbers{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:8px!important;margin:10px 0!important}.student-mini-stats article,.student-home-grid article,.student-ai-grid article,.attendance-discipline-grid article{padding:10px!important;break-inside:avoid}.student-score-ring,.attendance-discipline-rate,.goal-ring{border:2px solid #0b736b!important;background:#fff!important;color:#111!important;box-shadow:none!important}
table{width:100%!important;min-width:0!important;border-collapse:collapse!important;table-layout:fixed!important}th,td{border:1px solid #9fb1b7!important;padding:7px!important;text-align:right!important;font-size:11px!important;color:#111!important}th{background:#eaf4f2!important}.student-table-scroll{overflow:visible!important;border:0!important}h1,h2,h3,p,span,strong,small,li,td,th{color:#111!important;text-shadow:none!important}footer{text-align:center;margin-top:14px;padding-top:10px;border-top:1px solid #b9c8cc;font-size:11px;color:#60727a}
...
function clearSessionOnly() {
  try {
    window.sessionStorage.removeItem("lahooni-student-active");
    window.sessionStorage.removeItem("lahooni-active-student");
    window.localStorage.removeItem("lahooni-student-last-path");
    window.localStorage.removeItem("lahooni-active-student");
    window.localStorage.removeItem("lahooni-auth-session");
  } catch {
    // Server logout and hard redirect still complete the action.
  }
}

function logoutDestination(pathname: string) {
  if (pathname.startsWith("/teacher")) return "/teacher";
  if (pathname.startsWith("/admin")) return "/admin";
  return "/student";
}

function forceLogout(pathname: string) {
  clearSessionOnly();
  ...
  const redirect = () => {
    ...
    redirected = true;
    window.location.replace(`${logoutDestination(pathname)}?logout=${Date.now()}`);
  };
...
export default function PortalCommandRuntime() {
  useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      if (command === "logout" && /^(\/student|\/parent|\/family|\/teacher|\/admin)/.test(pathname)) {
        event.preventDefault();
        event.stopPropagation();
        event.stopImmediatePropagation();
