...
export default function DisciplinePage(){
  ...
  const scopeLabel=scope==="all"?"جميع الفصول":scope==="highRisk"?"نسبة المخالفة ٤٠٪ فأكثر":scope==="student"?(rows.find(r=>r.code===selectedStudent)?.name||"طالب محدد"):(selectedClass||"فصل محدد");
  const table=<table className="discipline-report-table"><thead><tr><th>#</th><th>الطالب</th><th>الفصل</th><th>الحصص</th><th>الغياب</th><th>التأخير</th><th>الهروب</th><th>الاستئذان</th><th>نسبة المخالفة</th><th>الحالة</th></tr></thead><tbody>{visible.map((r,i)=><tr key={r.code}><td>{ar(i+1)}</td><td><b>{r.name}</b><small>{r.code}</small></td><td>{r.className}</td><td>{ar(r.scheduledLessons)}</td><td className="abs">{ar(r.absent)}</td><td className="late">{ar(r.late)}</td><td className="esc">{ar(r.escaped)}</td><td className="exc">{ar(r.excused)}</td><td className={r.nonAttendanceRate>=40?"nonatt-high":"nonatt"}>{r.scheduledLessons?`${ar(r.nonAttendanceRate)}٪`:"—"}</td><td><span className={r.escaped>0||r.absent>=3||r.late>=3?"risk":r.total?"watch":"clear"}>{r.escaped>0||r.absent>=3||r.late>=3?"يحتاج متابعة":r.total?"ملاحظة":"منتظم"}</span></td></tr>)}</tbody></table>;
  ...
  return <main className="discipline-safe" dir="rtl"><div className="screen">
    ...
    <section className="table-card"><header><div><small>السجل التفصيلي</small><h2>{reportLabels[reportType]}</h2></div><span>{ar(visible.length)} طالب</span></header><div className="table-wrap">{table}</div></section>
      ...
      <div className="report-watermark">لـحـونـي</div>
      <header className="report-cert-head"><img src="/icons/ostadh-lahooni-192.jpg" alt="شعار البوابة"/><div><small>بوابة أستاذ لحوني التعليمية</small><h1>تقرير الانضباط الطلابي</h1><span>{session.subject||"—"} • {scopeLabel}</span></div><b>◈</b></header>
      <section className="report-identity"><div><small>معلم المادة</small><b>{session.teacherName||"—"}</b></div><div><small>المادة</small><b>{session.subject||"—"}</b></div><div><small>نطاق التقرير</small><b>{scopeLabel}</b></div><div><small>الفترة</small><b>{prettyDate(firstRecordedDate)} — {prettyDate(reportEnd)}</b></div></section>
      <section className="report-hero"><div><small>نسبة المخالفة المعتمدة</small><h2>٤٠٪ فأكثر</h2><p>الغياب + التأخير + الهروب + الاستئذان من إجمالي حصص المادة</p></div><div className="report-hero-count"><b>{ar(over40)}</b><span>طلاب يحتاجون متابعة</span></div></section>
      <section className="report-kpis"><div><small>إجمالي الطلاب</small><b>{ar(visible.length)}</b></div><div className="k-abs"><small>الغياب</small><b>{ar(totals.absent)}</b></div><div className="k-late"><small>التأخير</small><b>{ar(totals.late)}</b></div><div className="k-esc"><small>الهروب</small><b>{ar(totals.escaped)}</b></div><div className="k-exc"><small>الاستئذان</small><b>{ar(totals.excused)}</b></div></section>
      {false?classes.map(c=><section className="report-table-section print-class-page" key={c}><header><div><small>تفاصيل التقرير</small><h2>{c}</h2></div><span>{ar(visible.filter(r=>r.className===c).length)} طالب</span></header><table className="discipline-report-table"><thead><tr><th>#</th><th>الطالب</th><th>الفصل</th><th>الحصص</th><th>الغياب</th><th>التأخير</th><th>الهروب</th><th>الاستئذان</th><th>نسبة المخالفة</th><th>الحالة</th></tr></thead><tbody>{visible.filter(r=>r.className===c).map((r,i)=><tr key={r.code}><td>{ar(i+1)}</td><td><b>{r.name}</b><small>{r.code}</small></td><td>{r.className}</td><td>{ar(r.scheduledLessons)}</td><td className="abs">{ar(r.absent)}</td><td className="late">{ar(r.late)}</td><td className="esc">{ar(r.escaped)}</td><td className="exc">{ar(r.excused)}</td><td className={r.nonAttendanceRate>=40?"nonatt-high":"nonatt"}>{r.scheduledLessons?`${ar(r.nonAttendanceRate)}٪`:"—"}</td><td><span className={r.nonAttendanceRate>=40?"risk":r.total?"watch":"clear"}>{r.nonAttendanceRate>=40?"يحتاج متابعة":r.total?"ملاحظة":"منتظم"}</span></td></tr>)}</tbody></table></section>):<section className="report-table-section print-class-page"><header><div><small>تفاصيل التقرير</small><h2>{scopeLabel}</h2></div><span>{ar(visible.length)} طالب</span></header>{table}</section>}
      <footer className="report-cert-footer"><div><small>بيان التقرير</small><p>تقرير رسمي صادر من بوابة أستاذ لحوني التعليمية وفق سجلات الانضباط وحصص المادة.</p></div><div className="report-seal"><span>بوابة</span><b>أستاذ<br/>لحوني</b><small>تقرير معتمد</small></div><div className="report-sign"><small>معلم المادة</small><b>{session.teacherName||"—"}</b><span>{session.subject||"—"}</span><em>التوقيع: ........................</em></div></footer>
    </section></main>;
}
