...
export default function DisciplinePage(){
  ...
  </div><section className={`discipline-print ${printMode==="classPages"?"print-all-classes":"print-single-report"}`}><header><img src="/icons/ostadh-lahooni-192.jpg" alt="شعار البوابة"/><small>بوابة أستاذ لحوني التعليمية</small><h1>{reportLabels[reportType]}</h1><p>المعلم: {session.teacherName||"—"} · المادة: {session.subject||"—"} · النطاق: {scopeLabel}</p><p>الفترة: {prettyDate(firstRecordedDate)} — {prettyDate(reportEnd)}</p></header><div className="print-summary"><span>إجمالي الطلاب: <b>{ar(visible.length)}</b></span><span>عدم الالتزام ٤٠٪ فأكثر: <b>{ar(over40)}</b></span><span>الغياب: <b>{ar(totals.absent)}</b></span><span>التأخير: <b>{ar(totals.late)}</b></span><span>الهروب: <b>{ar(totals.escaped)}</b></span><span>الاستئذان: <b>{ar(totals.excused)}</b></span></div>{printMode==="classPages"?classes.map(c=><section className="print-class-page" key={c}><h2>{c}</h2><table className="discipline-report-table"><thead><tr><th>#</th><th>الطالب</th><th>الحصص</th><th>الغياب</th><th>التأخير</th><th>الهروب</th><th>الاستئذان</th><th>عدم الالتزام</th><th>الحالة</th></tr></thead><tbody>{visible.filter(r=>r.className===c).map((r,i)=><tr key={r.code}><td>{ar(i+1)}</td><td><b>{r.name}</b><small>{r.code}</small></td><td>{ar(r.scheduledLessons)}</td><td className="abs">{ar(r.absent)}</td><td className="late">{ar(r.late)}</td><td className="esc">{ar(r.escaped)}</td><td className="exc">{ar(r.excused)}</td><td className={r.nonAttendanceRate>=40?"nonatt-high":"nonatt"}>{r.scheduledLessons?`${ar(r.nonAttendanceRate)}٪`:"—"}</td><td><span className={r.nonAttendanceRate>=40?"risk":r.total?"watch":"clear"}>{r.nonAttendanceRate>=40?"يحتاج متابعة":r.total?"ملاحظة":"منتظم"}</span></td></tr>)}</tbody></table></section>):table}<footer>بوابة أستاذ لحوني التعليمية · {new Intl.DateTimeFormat("ar-SA",{dateStyle:"long",timeStyle:"short"}).format(new Date())}</footer></section></main>;
}
