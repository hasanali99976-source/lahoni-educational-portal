...
export default function DisciplinePage(){
  ...
  const rows=useMemo<Row[]>(()=>students.map(student=>{
    ...
    const lessons=scheduledLessons(className);const total=absent+late+escaped+excused;const absenceRate=lessons?Math.min(100,Math.round(absent/lessons*1000)/10):0;
  ...
  const affected=visible.filter(r=>r.total>0).length;const repeated=visible.filter(r=>r.total>=2).length;const urgent=visible.filter(r=>r.escaped>0||r.absent>=3||r.late>=3).length;const avgAbsence=visible.length?visible.reduce((s,r)=>s+r.absenceRate,0)/visible.length:0;
  ...
  const table=<table><thead><tr><th>#</th><th>الطالب</th><th>الفصل</th><th>الحصص</th><th>الغياب</th><th>التأخير</th><th>الهروب</th><th>الاستئذان</th><th>نسبة الغياب</th><th>الحالة</th></tr></thead><tbody>{visible.map((r,i)=><tr key={r.code}><td>{ar(i+1)}</td><td><b>{r.name}</b><small>{r.code}</small></td><td>{r.className}</td><td>{ar(r.scheduledLessons)}</td><td className="abs">{ar(r.absent)}</td><td className="late">{ar(r.late)}</td><td className="esc">{ar(r.escaped)}</td><td className="exc">{ar(r.excused)}</td><td>{r.scheduledLessons?`${ar(r.absenceRate)}٪`:"—"}</td><td><span className={r.escaped>0||r.absent>=3||r.late>=3?"risk":r.total?"watch":"clear"}>{r.escaped>0||r.absent>=3||r.late>=3?"يحتاج متابعة":r.total?"ملاحظة":"منتظم"}</span></td></tr>)}</tbody></table>;
  ...
  </div><section className="discipline-print"><header><small>بوابة أستاذ لحوني التعليمية</small><h1>{reportLabels[reportType]}</h1><p>المعلم: {session.teacherName||"—"} · المادة: {session.subject||"—"} · النطاق: {scopeLabel}</p><p>الفترة: {prettyDate(firstRecordedDate)} — {prettyDate(reportEnd)}</p></header>{table}<footer>بوابة أستاذ لحوني التعليمية · {new Intl.DateTimeFormat("ar-SA",{dateStyle:"long",timeStyle:"short"}).format(new Date())}</footer></section></main>;
}
