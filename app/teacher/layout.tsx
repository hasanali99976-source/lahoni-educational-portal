...
export default function TeacherLayout({ children }: { children: ReactNode }) {
  ...
  async function changeSubject(next:string){if(next===workspaceKey||switchingSubject)return;try{setSwitchingSubject(true);const response=await fetch("/api/teacher-session",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({workspaceKey:next}),cache:"no-store",credentials:"same-origin"});if(!response.ok)throw new Error();const switched=await response.json().catch(()=>null);if(!switched?.ok||!switched?.subjectId)throw new Error();setSubjectKey(switched.subjectId as SubjectKey);setWorkspaceKey(switched.workspaceKey||next);setActiveGrade(switched.activeGrade||null);setActiveGradeLabel(switched.activeGradeLabel||"");setSubjectName(getSubjectConfig(switched.subjectId).label);setMenuOpen(false);window.location.replace(pathname);}finally{setSwitchingSubject(false);}}
