...
export default function TeacherLayout({ children }: { children: ReactNode }) {
  ...
  return <TeacherClientContext.Provider value={providerValue}><div className={`teacher-app-shell ${subjectConfig.themeClass}`} dir="rtl" data-subject={subjectKey}><aside className="teacher-sidebar"><div className="teacher-shell-brand"><img className="teacher-portal-logo" src="/icons/ostadh-lahooni-192.jpg" alt="شعار بوابة أستاذ لحوني التعليمية"/><div><strong>بوابة أستاذ لحوني التعليمية</strong><small>{teacherName} • {subjectConfig.label}</small></div></div><div className="teacher-nav-title">مهام المعلم</div><nav className="teacher-tabs" aria-label="أقسام بوابة المعلم">{tabs.map(tab=>{const active=pathname.startsWith(tab.href);return <Link key={tab.href} href={tab.href} className={`${active?"active":""} ${tab.key==="ai"?"ai-tab":""}`} onClick={()=>playTone()}><span className="teacher-tab-icon"><TabIcon type={tab.key}/></span><span className="teacher-tab-copy"><b>{tab.label}</b><small>{tab.note}</small></span>{tab.badge&&<em>{tab.badge}</em>}</Link>})}</nav><div className="teacher-header-actions"><button className={`sound-toggle ${soundOn?"on":"off"}`} onClick={toggleSound}>{soundOn?"🔊 الصوت مفعل":"🔇 الصوت مكتوم"}</button><button className="teacher-logout" onClick={logout}>تسجيل خروج</button></div></aside><main className="teacher-main"><section className="teacher-welcome-strip"><div className="teacher-welcome-copy"><span className="teacher-welcome-badge">ملف المعلم — {subjectConfig.label}</span><h2>أهلًا أستاذ {teacherName}</h2><p>كل أدوات الإدارة والرصد والتقارير والذكاء الاصطناعي في مكان واحد واضح.</p><div className="teacher-welcome-points">{subjectConfig.welcomePoints.map(point=><span key={point}>{point}</span>)}</div></div><Link className="teacher-ai-quick" href="/teacher/ai"><span>AI</span><div><b>المساعد الذكي</b><small>تحليل النتائج • خطة علاجية</small></div></Link></section><div className="teacher-page-content">{children}</div></main></div></TeacherClientContext.Provider>
}
