...
type TeacherSession = { authenticated?: boolean; teacherId?: string; teacherName?: string; subject?: string; subjectKey?: SubjectKey; subjects?: TeacherSubject[]; firebaseToken?: string };
...
export default function TeacherLayout({ children }: { children: ReactNode }) {
  ...
  const [subjects,setSubjects]=useState<TeacherSubject[]>([]),[switchingSubject,setSwitchingSubject]=useState(false);
  ...
  useEffect(()=>{if(isLoginPage){setReady(true);return}let active=true,busy=false;const applySession=(session:TeacherSession)=>{setTeacherId(session.teacherId);setTeacherName(session.teacherName||"المعلم");setSubjectKey(session.subjectKey||"history");setSubjects(Array.isArray(session.subjects)?session.subjects:[])};const check=async()=>{const response=await fetch("/api/teacher-session",{cache:"no-store"});if(!response.ok)throw new Error();const session=await response.json() as TeacherSession;if(active){applySession(session);setReady(true)}};const reset=()=>{if(idleTimer.current)clearTimeout(idleTimer.current);idleTimer.current=setTimeout(()=>void logout(),IDLE_LIMIT)};const activity=()=>{reset();const now=Date.now();if(now-lastHeartbeat.current<30000||busy)return;busy=true;fetch("/api/teacher-session",{cache:"no-store"}).then(async response=>{if(!response.ok)throw new Error();const session=await response.json() as TeacherSession;if(active)applySession(session);lastHeartbeat.current=Date.now()}).catch(()=>void logout()).finally(()=>busy=false)};check().catch(()=>active&&router.replace("/teacher"));reset();const events=["pointerdown","keydown","touchstart","scroll","mousemove"];events.forEach(eventName=>window.addEventListener(eventName,activity,{passive:true}));return()=>{active=false;if(idleTimer.current)clearTimeout(idleTimer.current);events.forEach(eventName=>window.removeEventListener(eventName,activity))}},[isLoginPage,pathname,router]);
  async function changeSubject(subjectId:string){if(subjectId===subjectKey||switchingSubject)return;try{setSwitchingSubject(true);const response=await fetch('/api/teacher-session',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({subjectId}),cache:'no-store'});if(!response.ok)throw new Error();setSubjectKey(subjectId as SubjectKey);router.push('/teacher/dashboard');router.refresh()}finally{setSwitchingSubject(false)}}
  if(isLoginPage)return <>{children}</>;
  if(!ready)return <main className="teacher-shell-loading"><span className="loading-orbit"/>جارٍ تجهيز بوابة المعلم...</main>;
  const providerValue={authenticated:true,teacherId,teacherName,subjectKey,subject:subjectConfig.label,setSubject:changeSubject,refresh:async()=>{try{const r=await fetch('/api/teacher-session',{cache:'no-store'});if(!r.ok)return;const s=await r.json();setTeacherId(s.teacherId);setTeacherName(s.teacherName||'المعلم');setSubjectKey(s.subjectKey||'history');setSubjects(Array.isArray(s.subjects)?s.subjects:[])}catch{}}};
  return <TeacherClientContext.Provider value={providerValue}><div className={`teacher-app-shell ${subjectConfig.themeClass}`} dir="rtl" data-subject={subjectKey}><aside className="teacher-sidebar"><div className="teacher-shell-brand"><Image className="teacher-portal-logo" src="/icons/ostadh-lahooni-192.jpg" alt="شعار بوابة أستاذ لحوني التعليمية" width={54} height={54} priority/><div><strong>بوابة أستاذ لحوني التعليمية</strong><small>{teacherName}</small></div></div><section className="teacher-subject-switcher" aria-label="تغيير المادة"><div className="subject-switcher-icon" aria-hidden="true">{subjectConfig.shortMark}</div><label><span>المادة الحالية</span><select value={subjectKey} onChange={event=>void changeSubject(event.target.value)} disabled={switchingSubject||subjects.length<2}>{subjects.length?subjects.map(subject=><option key={subject.subjectId} value={subject.subjectId}>{subject.subjectName}</option>):<option value={subjectKey}>{subjectConfig.label}</option>}</select></label>{subjects.length<2?<small>المادة محددة من مدير البوابة</small>:<small>{switchingSubject?"جارٍ فتح المادة…":"غيّر بين المواد التي فعّلها المدير"}</small>}</section><div className="teacher-nav-title">أقسام بوابة المعلم</div><nav className="teacher-tabs" aria-label="أقسام بوابة المعلم">{tabs.map(tab=>{const active=pathname.startsWith(tab.href);return <Link key={tab.href} href={tab.href} className={`${active?"active":""} ${tab.key==="ai"?"ai-tab":""}`} onClick={()=>playTone()}><span className="teacher-tab-icon"><TabIcon type={tab.key}/></span><span className="teacher-tab-copy"><b>{tab.label}</b><small>{tab.note}</small></span>{tab.badge&&<em>{tab.badge}</em>}</Link>})}</nav><div className="teacher-header-actions"><button className={`sound-toggle ${soundOn?"on":"off"}`} onClick={toggleSound}>{soundOn?"🔊 الصوت مفعل":"🔇 الصوت مكتوم"}</button><button className="teacher-logout" onClick={logout}>تسجيل خروج</button></div></aside><main className="teacher-main"><section className="teacher-welcome-strip"><div className="teacher-welcome-copy"><span className="teacher-welcome-badge">مساحة {subjectConfig.label}</span><h2>أهلًا أستاذ {teacherName}</h2><p>إدارة الطلاب والدرجات والحضور والتقارير في مساحة تعليمية واحدة واضحة.</p><div className="teacher-welcome-points">{subjectConfig.welcomePoints.map(point=><span key={point}>{point}</span>)}</div></div><Link className="teacher-ai-quick" href="/teacher/ai"><span>AI</span><div><b>المساعد التعليمي الذكي</b><small>تحليل النتائج • خطط علاجية مقترحة</small></div></Link></section><div className="teacher-page-content">{children}</div></main></div></TeacherClientContext.Provider>
}
