...
export default function TeacherLayout({ children }: { children: ReactNode }) {
  ...
  async function logout(){try{window.speechSynthesis?.cancel();sessionStorage.removeItem("lahooni:teacher-welcome");}catch{}setReady(false);clearSessionState();try{await Promise.all([fetch("/api/teacher-logout",{method:"POST",cache:"no-store"}),signOut(auth)]);}finally{window.location.replace("/teacher");}}
  ...
  return <TeacherClientContext.Provider key={`${teacherId||"teacher"}:${workspaceKey}`} value={contextValue}><div className={`teacher-app-shell ${subjectConfig.themeClass} ${menuOpen?"menu-open":""}`} dir="rtl" data-subject={subjectKey}><header className="teacher-pro-header"><div className="teacher-pro-brand"><Image className="teacher-pro-logo" src="/icons/ostadh-lahooni-192.jpg" alt="شعار بوابة أستاذ لحوني التعليمية" width={43} height={43} priority/><div className="teacher-pro-brand-copy"><strong>بوابة أستاذ لحوني التعليمية</strong><small>{teacherName}</small></div></div><div className="teacher-pro-subject"><span className="teacher-pro-subject-mark">{subjectConfig.shortMark}</span>{subjects.length>1?<select aria-label="تغيير المادة أو المرحلة" value={workspaceKey} onChange={e=>void changeSubject(e.target.value)} disabled={switchingSubject}>{subjects.map(s=><option key={s.workspaceKey} value={s.workspaceKey}>{s.subjectName}{s.gradeLabel?` — ${s.gradeLabel}`:""}</option>)}</select>:<div className="teacher-pro-subject-copy"><small>المادة الحالية</small><strong>{subjectName}{activeGradeLabel?` — ${activeGradeLabel}`:""}</strong></div>}</div><nav className="teacher-pro-tabs" aria-label="التنقل الرئيسي">{primaryTabs.map(renderHeaderTab)}<button className={moreActive||menuOpen?"active teacher-pro-more":"teacher-pro-more"} onClick={()=>setMenuOpen(v=>!v)} aria-expanded={menuOpen}><TabIcon type="more"/><span>القائمة</span></button></nav><div className="teacher-pro-tools"><span className="teacher-pro-date">{todayLabel}</span><button type="button" className="teacher-sound-toggle" onClick={()=>speakTeacherWelcome()}>🔊 الترحيب</button><Link prefetch={false} href="/" className="teacher-pro-home">الرئيسية</Link><button onClick={logout} className="teacher-pro-logout">خروج</button></div></header><button type="button" className="teacher-command-backdrop" aria-label="إغلاق القائمة" onClick={()=>setMenuOpen(false)}/><aside className="teacher-command-panel" aria-label="كل خدمات المعلم"><div className="teacher-command-head"><div><small>مركز الأدوات</small><strong>كل خدمات المعلم</strong></div><button onClick={()=>setMenuOpen(false)} aria-label="إغلاق">×</button></div><div className="teacher-command-subject"><span>{subjectConfig.shortMark}</span><div><small>مساحة العمل الحالية</small><b>{subjectName}{activeGradeLabel?` — ${activeGradeLabel}`:""}</b></div></div><nav className="teacher-command-links">{primaryTabs.map(renderCommandTab)}{moreTabs.map(renderCommandTab)}</nav><div className="teacher-command-foot"><Link prefetch={false} href="/">العودة للرئيسية</Link><button onClick={logout}>تسجيل الخروج</button></div></aside><aside className="teacher-reference-sidebar"><div className="teacher-reference-side-brand"><Image src="/icons/ostadh-lahooni-192.jpg" alt="" width={34} height={34}/><div><b>بوابة المعلم</b><small>{teacherName}</small></div></div><nav>{sideTabs.map(tab=>{const active=pathname.startsWith(tab.href);return <Link prefetch={false} key={tab.href} href={tab.href} className={active?"active":""}><TabIcon type={tab.key}/><span>{tab.label}</span></Link>})}</nav><div className="teacher-reference-side-foot"><span>بالعلم .. نصنع المستقبل</span><button type="button" onClick={logout} className="teacher-reference-logout" aria-label="تسجيل الخروج"><span aria-hidden="true">↪</span> تسجيل الخروج</button></div></aside><main className="teacher-pro-main"><div className="teacher-reference-topbar"><div><b>{teacherName}</b><span>لوحة المعلم</span></div><div className="teacher-reference-subjects">{subjects.length>1?<select aria-label="المادة الحالية" value={workspaceKey} onChange={e=>void changeSubject(e.target.value)} disabled={switchingSubject}>{subjects.map(s=><option key={s.workspaceKey} value={s.workspaceKey}>{s.subjectName}{s.gradeLabel?` — ${s.gradeLabel}`:""}</option>)}</select>:<><strong>{subjectName}</strong><small>{activeGradeLabel||"المرحلة المسندة"}</small></>}</div></div><div className="teacher-pro-context"><div><small>مساحة المعلم</small><strong>{subjectName}</strong><span>{activeGradeLabel||"المرحلة المسندة"}</span></div><div className="teacher-pro-context-actions">{hasGradePlan?<span className="teacher-pro-ready">الخطة الدراسية جاهزة</span>:<Link prefetch={false} href="/teacher/grade-plan">إعداد الخطة الدراسية</Link>}</div></div><div className="teacher-page-content">{children}</div></main></div></TeacherClientContext.Provider>;
}
