...
function nextCode(used:Set<string>,className:string) {
  ...
  for (let number=1; number<=999; number++) { const candidate = `${prefix}${String(number).padStart(3,"0")}`; if (!used.has(candidate)) return candidate; }
  return "";
}
...
export default function QuotaStudentFallback() {
  ...
  useEffect(()=>{
    if (!teacherId) return;
    try { const stored = JSON.parse(localStorage.getItem(storageKey)||"[]"); setPending(Array.isArray(stored)?stored:[]); }
    catch { setPending([]); }
  },[storageKey,teacherId]);
  ...
  useEffect(()=>{
    const handleAdd = (event:Event)=>{
      ...
      const duplicate = pending.some(item=>normalizeArabic(item.name)===normalizeArabic(studentName)&&normalizeArabic(item.className)===normalizeArabic(className));
      ...
      savePending([...pending,{id:crypto.randomUUID(),name:studentName,className,code,createdAt:new Date().toISOString()}]);
  ...
  const syncPending = useCallback(async()=>{
    ...
    try {
      const [localSnapshot,sharedSnapshot] = await Promise.all([getDocs(collection(db,studentsPath)),getDocs(collection(db,SHARED_STUDENTS))]);
      ...
      for (const item of [...pending]) {
        const grade=gradeNumber(item.className); if(!grade) continue;
        const code=codePattern.test(item.code)&&!used.has(item.code)?item.code:nextCode(used,item.className); if(!code) continue;
        used.add(code);
        await setDoc(doc(db,classesPath,classId(item.className)),{name:item.className,teacherId,teacherName,subjectKey,updatedAt:serverTimestamp()},{merge:true});
        await setDoc(doc(db,SHARED_STUDENTS,item.id),{name:item.name,class:item.className,grade,accessCode:code,studentCode:code,active:true,firstTeacherId:teacherId,firstTeacherName:teacherName,lastTeacherId:teacherId,lastTeacherName:teacherName,updatedAt:serverTimestamp()},{merge:true});
        await setDoc(doc(db,studentsPath,code),{name:item.name,class:item.className,grade,accessCode:code,studentCode:code,teacherId,teacherName,subjectKey,subject,sharedRosterId:item.id,rosterActive:true,attendance:0,homework:0,participation:0,research:0,tests:[0,0,0,0,0],createdAt:serverTimestamp(),updatedAt:serverTimestamp()},{merge:true});
        const index=remaining.findIndex(entry=>entry.id===item.id); if(index>=0) remaining.splice(index,1); savePending([...remaining]);
      }
      setNotice(remaining.length?"بقيت أسماء لم تُرفع بعد.":"تم رفع جميع الأسماء إلى البوابة.");
    } catch { setNotice("الأسماء موجودة الآن داخل القوائم بأكوادها، وتعذر فقط رفعها للخادم حاليًا."); }
  ...
  const rows = tableTarget ? createPortal(<>{visiblePending.map((student,index)=><tr key={`pending-${student.id}`}>
    <td>{index+1}</td><td><strong>{student.name}</strong></td><td>{student.className}</td><td><button className="code-button" type="button">{student.code}</button></td>
