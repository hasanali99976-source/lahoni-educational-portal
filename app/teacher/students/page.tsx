...
function classDocId(name: string) { return encodeURIComponent(name.replace(/\//g, "-")).slice(0, 120); }
...
export default function StudentsPage() {
  ...
  useEffect(() => onSnapshot(collection(db, "students"), snap => {
    ...
    if (!migrationStarted.current) {
      ...
      Promise.all(updates).catch(() => setMessage("تعذر تحديث أكواد بعض الطلاب تلقائيًا"));
    }
  }), []);
  ...
  const classes = useMemo(() => {
    ...
    savedClasses.forEach(item => { const className = secondSecondaryName(item.name); if (className) counts.set(className, counts.get(className) || 0); });
    students.forEach(student => { const className = secondSecondaryName(student.class) || "غير محدد"; counts.set(className, (counts.get(className) || 0) + 1); });
  ...
  async function addClass() {
    ...
    try { await ensureClass(clean); setNewClassName(""); setMessage(`تمت إضافة الفصل ${clean}`); } catch { setMessage("تعذر إضافة الفصل"); }
  }
  async function renameClass(oldName: string) {
    ...
    try {
      ...
      await Promise.all(students.filter(student => secondSecondaryName(student.class) === oldName).map(student => updateDoc(doc(db, "students", student.id), { class: clean })));
      const saved = savedClasses.find(item => secondSecondaryName(item.name) === oldName); if (saved) await deleteDoc(doc(db, "classes", saved.id));
  ...
  async function removeClass(className: string) {
    if (students.some(student => secondSecondaryName(student.class) === className)) return setMessage("لا يمكن حذف فصل يحتوي على طلاب");
    if (!window.confirm(`هل تريد حذف الفصل ${className}؟`)) return;
    const saved = savedClasses.find(item => secondSecondaryName(item.name) === className); if (saved) await deleteDoc(doc(db, "classes", saved.id));
  ...
  async function saveStudent() {
    ...
    try {
      ...
      if (editingId) {
        await updateDoc(doc(db, "students", editingId), { name: name.trim(), nationalId, class: finalClass, accessCode });
        setMessage(`تم تعديل بيانات الطالب وتحديث الكود إلى ${accessCode}`);
      } else {
        await setDoc(doc(db, "students", nationalId), {
          name: name.trim(), nationalId, class: finalClass, accessCode,
          attendance: 0, homework: 0, participation: 0, research: 0,
          tests: [0, 0, 0, 0, 0], createdAt: serverTimestamp(),
        }, { merge: true });
        setMessage(`تمت إضافة الطالب وإنشاء الكود ${accessCode}`);
      }
  ...
  async function refreshCode(student: Student) {
    ...
    try { await updateDoc(doc(db, "students", student.id), { accessCode: code }); setMessage(`تم تثبيت الكود ${code}`); }
  ...
  async function importExcel(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      for (const item of unique) await setDoc(doc(db, "students", item.nationalId), {
        ...item, accessCode: generateAccessCode(item.nationalId), attendance: 0, homework: 0, participation: 0, research: 0,
        tests: [0, 0, 0, 0, 0], importedAt: serverTimestamp(),
      }, { merge: true });
      setMessage(`تم استيراد ${unique.length} طالبًا وإنشاء الأكواد تلقائيًا`);
  ...
  async function removeStudent(student: Student) {
    ...
    try { await deleteDoc(doc(db, "students", student.id)); setMessage("تم حذف الطالب وكوده"); } catch { setMessage("تعذر حذف الطالب"); }
  }

  return <main className="shell dashboard students-management"><div className="container">
    {!selectedClass ? <>
      <section className="card"><h1>إدارة فصول الصف الثاني الثانوي</h1><p>الأكواد تُنشأ تلقائيًا بصيغة TH + آخر 4 أرقام من هوية الطالب.</p><div className="form-grid"><input className="field" value={newClassName} onChange={e => setNewClassName(e.target.value)} placeholder="اسم فصل جديد مثل: ثاني ز" /><button className="btn primary" onClick={addClass}>إضافة فصل</button></div><div className="import-box"><div><strong>استيراد الطلاب من Excel</strong><p>ينشئ كود كل طالب تلقائيًا أثناء الاستيراد.</p></div><label className="btn secondary import-label">{importing ? "جارٍ الاستيراد..." : "اختيار ملف Excel"}<input type="file" accept=".xlsx,.xls" onChange={importExcel} disabled={importing} hidden /></label></div>{message && <p className="notice">{message}</p>}</section>
      ...
      <section className="access-code-print-sheet"><header><h1>مدرسة التهذيب الثانوية</h1><p>بطاقات دخول بوابة الطالب وولي الأمر — {selectedClass}</p><small>إعداد / الأستاذ حسن علي الطويل</small></header><div className="access-code-card-grid">{visibleStudents.map(student => { const code = student.accessCode || generateAccessCode(student.nationalId || student.id); return code ? <article key={student.id} className="access-code-card"><div className="code-card-copy"><span>بوابة الطالب وولي الأمر</span><h2>{student.name}</h2><p>{student.class}</p><strong>{code}</strong><small>TH + آخر 4 أرقام من الهوية</small></div><QRCodeSVG value={portalLink(code)} size={112} level="M" includeMargin /></article> : null; })}</div></section>
    </>}
  </div></main>;
}
