...
type Student={id:string;name?:string;nationalId?:string;class?:string;accessCode?:string;teacherId?:string;subjectKey?:string};
type SavedClass={id:string;name?:string;teacherId?:string;subjectKey?:string};
...
export default function StudentsPage(){
 ...
 const[name,setName]=useState("");const[nationalId,setNationalId]=useState("");const[studentClass,setStudentClass]=useState("");const[newClass,setNewClass]=useState("");const[search,setSearch]=useState("");const[editingId,setEditingId]=useState<string|null>(null);const[msg,setMsg]=useState("");const[busy,setBusy]=useState(false);const migrated=useRef(false);
 ...
 useEffect(()=>{if(!ready||!studentsPath||!classesPath)return;let cancelled=false;
  async function migrate(){if(migrated.current||teacherId!=="hasan-history"||subjectKey!=="history")return;migrated.current=true;try{const[ss,cc]=await Promise.all([getDocs(collection(db,"students")),getDocs(collection(db,"classes"))]);await Promise.all([...ss.docs.map(async x=>{const d=x.data() as Student;const owner=d.teacherId||"hasan-history",key=d.subjectKey||"history";if(owner!==teacherId||key!==subjectKey)return;const nid=id10(d.nationalId||x.id)||x.id;await setDoc(doc(db,studentsPath,nid),{...d,nationalId:d.nationalId||nid,teacherId,subjectKey,migratedAt:serverTimestamp()},{merge:true})}),...cc.docs.map(async x=>{const d=x.data() as SavedClass;const owner=d.teacherId||"hasan-history",key=d.subjectKey||"history";if(owner!==teacherId||key!==subjectKey)return;const n=className(d.name);if(n)await setDoc(doc(db,classesPath,classId(n)),{...d,name:n,teacherId,subjectKey,migratedAt:serverTimestamp()},{merge:true})})])}catch{if(!cancelled)setMsg("تعذر نسخ بعض البيانات القديمة، ولم يتم حذف أي بيانات.")}}
  migrate();const a=onSnapshot(collection(db,studentsPath),s=>setStudents(s.docs.map(x=>({id:x.id,...x.data()})) as Student[]),()=>setMsg("تعذر قراءة الطلاب"));const b=onSnapshot(collection(db,classesPath),s=>setClassesData(s.docs.map(x=>({id:x.id,...x.data()})) as SavedClass[]),()=>setMsg("تعذر قراءة الفصول"));return()=>{cancelled=true;a();b()}
 },[ready,studentsPath,classesPath,teacherId,subjectKey]);

 const classes=useMemo(()=>{const m=new Map<string,number>();classesData.forEach(x=>{const n=className(x.name);if(n)m.set(n,m.get(n)||0)});students.forEach(x=>{const n=className(x.class)||"غير محدد";m.set(n,(m.get(n)||0)+1)});return[...m].map(([name,count])=>({name,count})).sort((a,b)=>a.name.localeCompare(b.name,"ar"))},[students,classesData]);
 const visible=useMemo(()=>students.filter(x=>(!selectedClass||className(x.class)===selectedClass)&&(!search.trim()||(x.name||"").includes(search.trim())||(x.nationalId||"").includes(search.trim())||(x.accessCode||"").includes(search.trim()))),[students,selectedClass,search]);
 ...
 async function removeClass(n:string){
  const classStudents=students.filter(s=>className(s.class)===n);
  const warning=classStudents.length?`سيتم حذف الفصل ${n} وجميع طلابه وعددهم ${classStudents.length}، مع درجاتهم وبياناتهم داخل هذه المادة. لا يمكن التراجع. هل تريد المتابعة؟`:`حذف الفصل ${n}؟`;
  if(!window.confirm(warning))return;
  try{
   setBusy(true);setMsg("");
   const savedClasses=classesData.filter(x=>className(x.name)===n);
   await Promise.all([...classStudents.map(s=>deleteDoc(doc(db,studentsPath,s.id))),...savedClasses.map(c=>deleteDoc(doc(db,classesPath,c.id)))]);
   if(selectedClass===n)setSelectedClass(null);
   setMsg(classStudents.length?`تم حذف الفصل و${classStudents.length} طالبًا من مساحة ${subjectKey==="critical-thinking"?"التفكير الناقد":"التاريخ"}`:"تم حذف الفصل");
  }catch(error){console.error(error);setMsg("تعذر حذف الفصل أو بعض طلابه")}
