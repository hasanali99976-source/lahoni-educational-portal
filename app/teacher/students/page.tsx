...
export default function StudentsPage() {
  ...
  useEffect(() => onSnapshot(collection(db, "students"), snap => {
    ...
    if (!migrationStarted.current) {
      ...
      const oldClasses = list.filter(student => normalizeText(student.class).startsWith("أول"));
      Promise.all(oldClasses.map(student =>
        updateDoc(doc(db, "students", student.id), { class: secondSecondaryName(student.class) })
      )).catch(() => setMessage("تعذر تحويل بعض أسماء الفصول إلى الصف الثاني"));
    }
  }), []);
  ...
  async function addClass() {
    ...
    try {
      ...
      setNewClassName("");
      setMessage(`تمت إضافة الفصل ${clean}`);
    } catch {
      setMessage("تعذر إضافة الفصل");
    }
  }
  ...
  async function renameClass(oldName: string) {
    ...
    try {
      ...
      setMessage("تعذر تغيير اسم الفصل");
    }
  }
  ...
  async function removeClass(className: string) {
    ...
    if (count > 0) {
      setMessage("لا يمكن حذف فصل يحتوي على طلاب. انقل الطلاب أو احذفهم أولًا.");
      return;
    }
    ...
    try {
      ...
      for (const sheetName of workbook.SheetNames) {
        ...
        for (const row of rows.slice(headerIndex + 1)) {
          ...
          imported.push({ name: nameValue, nationalId: idValue, class: importedClass });
        }
      }
      ...
      for (const student of unique) {
        await setDoc(doc(db, "students", student.nationalId), {
          ...student, attendance: 0, homework: 0, participation: 0, research: 0,
          tests: [0, 0, 0, 0, 0], importedAt: serverTimestamp(),
        }, { merge: true });
      }
      setMessage(`تم استيراد ${unique.length} طالبًا في فصول الصف الثاني الثانوي`);
    } catch (error) {
      console.error(error);
  ...
  function openClass(className: string) {
    setSelectedClass(className); setStudentClass(className); setSearch(""); setMessage(""); setEditingId(null);
  }

  function startEdit(student: Student) {
    setEditingId(student.id); setName(student.name || ""); setNationalId(student.nationalId || "");
    setStudentClass(secondSecondaryName(student.class)); window.scrollTo({ top: 0, behavior: "smooth" });
  }
    ...
    <main className="shell dashboard"><div className="container">
      {!selectedClass ? <>
        <section className="card">
          <h1>إدارة فصول الصف الثاني الثانوي</h1>
          <p>يمكنك إضافة الفصول أو تغيير أسمائها أو حذف الفصل الفارغ مستقبلًا.</p>
          <div className="form-grid">
            <input className="field" value={newClassName} onChange={e => setNewClassName(e.target.value)} placeholder="اسم فصل جديد مثل: ثاني ز" />
            <button className="btn primary" onClick={addClass}>إضافة فصل</button>
          </div>
          <div className="import-box">
            <div><strong>استيراد الطلاب من Excel</strong><p>يحوّل أسماء أوراق «أول» تلقائيًا إلى «ثاني».</p></div>
            <label className="btn secondary import-label">{importing ? "جارٍ الاستيراد..." : "اختيار ملف Excel"}<input type="file" accept=".xlsx,.xls" onChange={importExcel} disabled={importing} hidden /></label>
          </div>
          {message && <p className="notice">{message}</p>}
        </section>
        <section className="class-grid" style={{ marginTop: 18 }}>
          {classes.map(item => <div key={item.name} className="class-card">
            <button className="class-open" onClick={() => openClass(item.name)}><span className="class-icon">📘</span><strong>{item.name}</strong><span>{item.count} طالبًا</span></button>
            <div className="class-actions"><button className="small-btn edit" onClick={() => renameClass(item.name)}>تغيير الاسم</button><button className="small-btn delete" onClick={() => removeClass(item.name)}>حذف الفصل</button></div>
          </div>)}
          {!classes.length && <section className="card"><p>لا توجد فصول حتى الآن.</p></section>}
        </section>
      </> : <>
        <section className="card"><div className="toolbar"><div><button className="small-btn edit" onClick={() => { setSelectedClass(null); setStudentClass(""); setSearch(""); }}>الرجوع إلى الفصول</button><h1 style={{ marginBottom: 0 }}>الفصل: {selectedClass}</h1><p>{visibleStudents.length} طالبًا</p></div><input className="field search" value={search} onChange={e => setSearch(e.target.value)} placeholder="بحث بالاسم أو الهوية" /></div></section>
        <section className="card" style={{ marginTop: 18 }}><h2>{editingId ? "تعديل بيانات الطالب" : "إضافة طالب إلى هذا الفصل"}</h2><div className="form-grid"><input className="field" value={name} onChange={e => setName(e.target.value)} placeholder="اسم الطالب" /><input className="field" inputMode="numeric" value={nationalId} onChange={e => setNationalId(e.target.value.replace(/\D/g, "").slice(0, 10))} placeholder="رقم الهوية" /><input className="field" value={studentClass} onChange={e => setStudentClass(e.target.value)} placeholder="الفصل" /></div><div className="button-row"><button className="btn primary" onClick={saveStudent} disabled={saving}>{saving ? "جارٍ الحفظ..." : editingId ? "حفظ التعديل" : "إضافة الطالب"}</button>{editingId && <button className="btn secondary" onClick={() => { setEditingId(null); setName(""); setNationalId(""); setStudentClass(selectedClass); }}>إلغاء</button>}</div>{message && <p className="notice">{message}</p>}</section>
        <section className="card" style={{ marginTop: 18 }}><div className="table-wrap"><table><thead><tr><th>#</th><th>اسم الطالب</th><th>رقم الهوية</th><th>الإجراءات</th></tr></thead><tbody>{visibleStudents.map((student, index) => <tr key={student.id}><td>{index + 1}</td><td>{student.name}</td><td>{student.nationalId}</td><td><button className="small-btn edit" onClick={() => startEdit(student)}>تعديل</button><button className="small-btn delete" onClick={() => removeStudent(student)}>حذف</button></td></tr>)}{!visibleStudents.length && <tr><td colSpan={4}>لا يوجد طلاب في هذا الفصل</td></tr>}</tbody></table></div></section>
      </>}
    </div></main>
  );
}
