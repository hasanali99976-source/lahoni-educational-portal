...
export default function TimetablePage(){
 ...
 async function saveLesson(){if(!selected||!timetablePath)return;if(!draft.className.trim())return setMessage("اختر الفصل قبل الحفظ.");const cellKey=keyFor(selected.day,selected.period),next={...schedule,[cellKey]:{subject:subject.label,className:draft.className.trim(),notes:draft.notes.trim()}};await persist(next,"تم حفظ الحصة");closeEditor()}
 ...
 return <main className="timetable-page" dir="rtl">
  ...
  <section className="mobile-days">{days.map(day=><article key={day.key}><h2>{day.label}</h2><div>{periods.map(period=>{const lesson=schedule[keyFor(day.key,period)];return <button key={period} className={lesson?"filled":""} onClick={()=>openCell(day.key,period)}><span>الحصة {ar.format(period)}</span>{lesson?<><strong>{subject.label}</strong><small>{lesson.className}</small></>:<b>إضافة الفصل</b>}</button>})}</div></article>)}</section>
  {selected&&<div className="lesson-modal no-print" role="dialog" aria-modal="true"><div className="lesson-editor"><header><div><span>تعديل الحصة {ar.format(selected.period)}</span><h2>{days.find(day=>day.key===selected.day)?.label}</h2></div><button className="close" onClick={closeEditor}>×</button></header><div className="fixed-subject"><span>المادة الحالية</span><strong>{subject.label}</strong><small>تُحدد تلقائيًا من بوابة المعلم</small></div><label><span>الفصل</span><select value={draft.className} onChange={event=>setDraft(current=>({...current,className:event.target.value}))}><option value="">اختر الفصل</option>{classes.map(className=><option key={className} value={className}>{className}</option>)}</select>{!classes.length&&<small>أضف الفصول أولًا من تبويب إدارة الطلاب.</small>}</label><label><span>ملاحظات اختيارية</span><textarea value={draft.notes} onChange={event=>setDraft(current=>({...current,notes:event.target.value}))} placeholder="قاعة، نشاط، اختبار..."/></label><footer><button className="save" onClick={saveLesson} disabled={saving}>{saving?"جارٍ الحفظ...":"حفظ الحصة"}</button><button className="delete" onClick={clearLesson} disabled={saving}>حذف الحصة</button><button onClick={closeEditor}>إلغاء</button></footer></div></div>}
 </main>
}
