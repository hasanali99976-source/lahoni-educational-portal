...
type Snapshot=Record<string,{studentId:string;studentName:string;value:number;sectionId:string;sectionLabel:string;itemId:string;itemLabel:string;maximum:number}>;

function valuesFor(student:StudentRow,plan:GradePlan){return student.gradePlanValues?.[plan.id]||student.gradeValues||{}}
function buildSnapshot(students:StudentRow[],plan:GradePlan):Snapshot{
  ...
  for(const student of students){
    ...
    for(const section of plan.sections)for(const item of section.items){
      ...
      out[`${studentId}::${key}`]={studentId,studentName:String(student.name||""),value:Number(values[key]||0),sectionId:section.id,sectionLabel:section.label,itemId:item.id,itemLabel:item.label,maximum:item.max};
    }
  }
  return out;
}
...
export default function GradeHistoryRecorder(){
  ...
  useEffect(()=>{
    ...
    void fetchStudents().then(rows=>{if(!cancelled)baseline.current=buildSnapshot(rows,activePlan)}).catch(()=>{});
      ...
      window.setTimeout(async()=>{
        try{
          ...
          for(const [snapKey,next] of Object.entries(after)){
            ...
            if(!prev||next.value===prev.value)continue;
            const delta=Number((next.value-prev.value).toFixed(2));
            ...
            writes.push(setDoc(doc(db,tenantStudentsPath(tenant),next.studentId),{
              gradeHistory:arrayUnion({
                id:eventId,planId:activePlan.id,planVersion:activePlan.version,teacherId:tenant.teacherId,teacherName:tenant.teacherName,subjectKey:tenant.subjectKey,
                sectionId:next.sectionId,sectionLabel:next.sectionLabel,itemId:next.itemId,itemLabel:next.itemLabel,maximum:next.maximum,
                before:prev.value,after:next.value,delta,changedAt:now,
              }),
        ...
        }catch(error){console.error("grade-history-recorder",error)}finally{busy.current=false}
      },1800);
    };
