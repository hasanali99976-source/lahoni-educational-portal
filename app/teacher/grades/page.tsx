...
export default function GradesPage(){
  ...
  function adjustedResult(student:Student){const result=calculateGradePlanResult(activePlan!,effectiveStudent(student));const deduction=totalDeductionFor(student);const earned=Math.max(0,roundGrade(result.earned-deduction));const maximum=Math.max(0,roundGrade(result.maximum));const percentage=maximum?Math.round((earned/maximum)*100):0;return{result,earned,percentage,deduction,maximum};}
  ...
  return <main className="grades-v11" dir="rtl">
    ...
    <section className="gv11-gradebook">
      ...
      <footer><span>خصم كل وحدة مستقل عنها؛ يظهر داخل نفس الوحدة في سجل الطالب ثم ينعكس تلقائيًا على إجمالي المادة ونسبة التحصيل.</span><span>{session.subject||"المادة"} • {selectedClass||"—"}</span></footer>
    </section>
  </main>;
}
