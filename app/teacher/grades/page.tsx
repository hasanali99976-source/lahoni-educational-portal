...
export default function GradesPage() {
  ...
  function printRegister() {
    ...
    if (!printable) return setMessage("اسمح بالنوافذ المنبثقة للطباعة");
    const rows = classStudents.map((student, index) => {
      const row = grades[student.id] || emptyGrade;
      return `<tr><td>${index + 1}</td><td>${student.name}</td><td>${row.attendance}</td><td>${row.participation}</td><td>${row.homework}</td><td>${row.unitExam}</td><td>${calculateUnitTotal(row)}</td><td>${row.notes || ""}</td></tr>`;
    }).join("");
    printable.document.write(`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><style>body{font-family:Arial}h1,p{text-align:center}table{width:100%;border-collapse:collapse}th,td{border:1px solid #222;padding:7px;text-align:center}</style></head><body><h1>سجل رصد الدرجات</h1><p>${session.subject || "المادة"} — ${session.activeGradeLabel || ""} — ${selectedClass}</p><table><thead><tr><th>م</th><th>اسم الطالب</th><th>الحضور</th><th>المشاركة</th><th>الواجبات</th><th>${unitInfo.examLabel}</th><th>المجموع</th><th>الملاحظات</th></tr></thead><tbody>${rows}</tbody></table><script>window.print()</script></body></html>`);
  ...
  return <main className="gradebook-page grades-page" dir="rtl"><div className="gradebook-wrap"><section className="gradebook-card">
    <header className="gradebook-head"><div><h1>سجل رصد الدرجات — {unitInfo.label}</h1><p>{session.subject || "المادة"}{session.activeGradeLabel ? ` — ${session.activeGradeLabel}` : ""}. تظهر الفصول الرقمية المختارة فقط.</p></div><div className="gradebook-actions"><label>الفصل<select value={selectedClass} onChange={event => setSelectedClass(event.target.value)}><option value="">اختر الفصل</option>{classes.map(name => <option key={name}>{name}</option>)}</select></label><label>الوحدة<select value={selectedUnit} onChange={event => setSelectedUnit(event.target.value as typeof selectedUnit)}>{ACADEMIC_UNITS.map(unit => <option key={unit.key} value={unit.key}>{unit.label}</option>)}</select></label><button type="button" className="research-link" onClick={printRegister}>🖨️ طباعة</button><button type="button" className="research-link" onClick={exportExcel}>📊 Excel</button><Link href="/teacher/research" className="research-link">🔬 درجة البحث</Link><button type="button" className="save-button" onClick={saveRegister} disabled={!selectedClass || saving}>{saving ? "جارٍ الحفظ..." : "💾 حفظ الدرجات"}</button></div></header>
