...
export default function ResearchPage() {
  ...
  return <main className="gradebook-page research-page" dir="rtl"><div className="gradebook-wrap"><section className="gradebook-card">
    <style>{`
      .research-page .research-mobile-list{display:none}
      .research-page .gradebook-scroll{overflow-x:auto;-webkit-overflow-scrolling:touch}
      .research-page .research-table{min-width:720px}
      .research-page .research-grade-link{display:inline-flex;align-items:center;justify-content:center;min-height:44px;padding:0 17px;border-radius:13px;background:#17263d;color:#fff;font-weight:900;text-decoration:none;white-space:nowrap;box-shadow:0 8px 20px rgba(23,38,61,.2)}
      .research-page .research-grade-link:active{transform:scale(.98)}
      @media(max-width:700px){
        .research-page .gradebook-card{padding:12px!important;border-radius:18px!important}
        .research-page .gradebook-head{display:block!important}
        .research-page .gradebook-head h1{font-size:21px!important}
        .research-page .gradebook-actions{display:grid!important;grid-template-columns:1fr 1fr;gap:9px!important;width:100%;margin-top:14px}
        .research-page .gradebook-actions label{grid-column:1/-1;width:100%}
        .research-page .gradebook-actions select{width:100%;min-height:48px}
        .research-page .gradebook-actions button,.research-page .research-grade-link{width:100%;min-height:48px;padding:8px;font-size:12px}
        .research-page .research-desktop-table{display:none!important}
        .research-page .research-mobile-list{display:grid!important;gap:10px;margin-top:14px}
        .research-page .research-student-card{display:grid;grid-template-columns:1fr auto;align-items:center;gap:12px;padding:13px;border:1px solid #dfe7ee;border-radius:16px;background:#fff;box-shadow:0 8px 22px rgba(20,46,70,.07)}
        .research-page .research-student-info{min-width:0}
        .research-page .research-student-info strong{display:block;font-size:14px;color:#17263d;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .research-page .research-student-info small{display:block;margin-top:4px;color:#64748b;font-size:10px}
        .research-page .mobile-grade-control{min-width:132px}
        .research-page .gradebook-footer{display:grid!important;grid-template-columns:1fr 1fr;gap:5px;font-size:10px}
      }
      @media(max-width:390px){.research-page .gradebook-actions{grid-template-columns:1fr}.research-page .gradebook-actions label{grid-column:auto}.research-page .research-student-card{grid-template-columns:1fr}.research-page .mobile-grade-control{width:100%}}
    `}</style>
    <header className="gradebook-head"><div><h1>رصد البحث — {subjectLabel}</h1><p>{tenant?.teacherName||"المعلم"} — درجة البحث من ٥ درجات وتُحفظ داخل مساحة المادة الحالية فقط.</p></div><div className="gradebook-actions"><label>الفصل<select value={selectedClass} onChange={e=>setSelectedClass(e.target.value)}><option value="">اختر الفصل</option>{classes.map(name=><option key={name}>{name}</option>)}</select></label><Link href="/teacher/grades" className="research-grade-link">📊 رصد الدرجات</Link><button type="button" className="save-button" onClick={applyAll} disabled={!selectedClass}>✓ تطبيق ٥ للجميع</button><button type="button" className="save-button" onClick={save} disabled={!selectedClass||saving}>{saving?"جارٍ الحفظ...":"💾 حفظ"}</button><button type="button" className="delete-all-button" onClick={clearAll} disabled={!selectedClass||saving}>🗑 حذف الكل</button></div></header>
    ...
    <div className="research-mobile-list">{classStudents.map((student,index)=><article className="research-student-card" key={student.id}><div className="research-student-info"><strong>{index+1}. {student.name}</strong><small>السجل المدني: {student.nationalId||"—"}</small></div><div className="mobile-grade-control"><button type="button" className="grade-step minus" onClick={()=>adjustScore(student.id,-1)}>−</button><input className="grade-input" aria-label={`درجة بحث ${student.name||"الطالب"}`} type="number" inputMode="numeric" min="0" max="5" step="1" value={scores[student.id]||0} onChange={e=>updateScore(student.id,e.target.value)} onFocus={e=>e.currentTarget.select()}/><button type="button" className="grade-step plus" onClick={()=>adjustScore(student.id,1)}>+</button></div></article>)}{!selectedClass&&<div className="empty-row">اختر الفصل لعرض الطلاب</div>}</div>
