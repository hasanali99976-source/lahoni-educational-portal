...
export default function ResearchPage() {
  ...
  function adjustScore(studentId:string,amount:number){ updateScore(studentId,String(Number(scores[studentId]||0)+amount)); }
  ...
  return <main className="gradebook-page research-page" dir="rtl"><div className="gradebook-wrap"><section className="gradebook-card">
    ...
    {selectedClass&&<div className="research-mobile-list">{classStudents.map((student,index)=><article className="research-mobile-card" key={student.id}><div><span>{index+1}</span><strong>{student.name}</strong><small>{student.nationalId||""}</small></div><div className="research-score-control"><button type="button" onClick={()=>adjustScore(student.id,-1)}>−</button><input type="number" inputMode="numeric" min="0" max="5" step="1" value={scores[student.id]||0} onChange={e=>updateScore(student.id,e.target.value)} onFocus={e=>e.currentTarget.select()}/><button type="button" onClick={()=>adjustScore(student.id,1)}>+</button><em>/ ٥</em></div></article>)}</div>}
    <div className="gradebook-scroll research-desktop-table"><table className="gradebook-table research-table"><thead><tr><th className="sticky-number">م</th><th className="national-id-head">السجل المدني</th><th className="sticky-name">اسم الطالب</th><th className="exam-head"><span>البحث</span><small>من ٥</small></th></tr></thead><tbody>{classStudents.map((student,index)=><tr key={student.id}><td className="sticky-number">{index+1}</td><td className="national-id-cell">{student.nationalId}</td><td className="sticky-name"><strong>{student.name}</strong></td><td className="exam-cell"><input className="grade-input" type="number" min="0" max="5" value={scores[student.id]||0} onChange={e=>updateScore(student.id,e.target.value)}/></td></tr>)}{!selectedClass&&<tr><td colSpan={4} className="empty-row">اختر الفصل لعرض الطلاب</td></tr>}</tbody></table></div>
