...
export default function AttendancePage(){
 ...
 const counts=useMemo(()=>{const v=classStudents.map(s=>records[s.id]||"present");return{present:v.filter(x=>x==="present").length,absent:v.filter(x=>x==="absent").length,late:v.filter(x=>x==="late").length,excused:v.filter(x=>x==="excused").length}},[classStudents,records]);
