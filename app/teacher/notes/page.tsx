...
export default function TeacherNotesPage(){
  ...
  function reportClasses(){return reportScope==="all"?classes:(className?[className]:[]);}
  function reportNoteTotal(){const selected=reportClasses();return rows.filter(row=>selected.includes(row.className)).reduce((sum,row)=>sum+(row.notes?.length||0),0);}
  function openPrint(){
    if(!reportNoteTotal())return setMessage("لا توجد ملاحظات في النطاق المحدد للطباعة.");
    setShowReport(true);setMessage("");
  ...
  return <main className="notes-v13" dir="rtl">
    ...
    <section className="nv13-classbar">
      ...
      <div className="nv13-export-tools">
        <label><span>طباعة الملاحظات</span><select value={reportScope} onChange={event=>setReportScope(event.target.value as "current"|"all")}><option value="current">الفصل المحدد</option><option value="all">جميع الفصول</option></select></label>
        <button type="button" onClick={()=>setShowReport(value=>!value)}>{showReport?"إخفاء التقرير":"عرض التقرير"}</button>
        <button type="button" onClick={openPrint}>طباعة / حفظ PDF</button>
      </div>
    </section>
