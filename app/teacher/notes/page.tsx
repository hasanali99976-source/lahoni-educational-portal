...
export default function TeacherNotesPage(){
  ...
  async function exportClassPdf(){
    const reportClasses=reportScope==="all"?classes:(className?[className]:[]);
    const total=rows.filter(row=>reportClasses.includes(row.className)).reduce((sum,row)=>sum+(row.notes?.length||0),0);
    if(!total)return setMessage("لا توجد ملاحظات في النطاق المحدد للتصدير.");
    setClassView(true);setPdfBusy(true);setMessage("");
    try{await new Promise(resolve=>setTimeout(resolve,100));const target=document.getElementById("nv13-class-report");if(!target)throw new Error();const html2canvas=(await import("html2canvas")).default;const{jsPDF}=await import("jspdf");const canvas=await html2canvas(target,{scale:1.8,backgroundColor:"#ffffff",useCORS:true});const pdf=new jsPDF({orientation:"portrait",unit:"mm",format:"a4"}),margin=7,w=196,h=canvas.height*w/canvas.width,pageH=283,data=canvas.toDataURL("image/png");for(let i=0;i<Math.max(1,Math.ceil(h/pageH));i++){if(i)pdf.addPage();pdf.addImage(data,"PNG",margin,margin-i*pageH,w,h);}pdf.save(reportScope==="all"?"ملاحظات-جميع-الفصول.pdf":`ملاحظات-${className}.pdf`);setMessage("تم إنشاء ملف PDF للملاحظات.");}catch{setMessage("تعذر إنشاء PDF الآن.");}finally{setPdfBusy(false);}
  }<div className="nv13-class-summary">
        <span><b>{classNotes}</b> ملاحظة</span>
        <span><b>{classFollowed}</b> طالب تمت متابعته</span>
        <button type="button" onClick={()=>setClassView(value=>!value)}>{classView?"إخفاء الملاحظات":"عرض الملاحظات"}</button>
      </div>
      <div className="nv13-export-tools">
        <label><span>نطاق التقرير</span><select value={reportScope} onChange={event=>setReportScope(event.target.value)}><option value="current">الفصل المحدد</option><option value="all">جميع الفصول</option></select></label>
        <button type="button" disabled={pdfBusy} onClick={()=>void exportClassPdf()}>{pdfBusy?"جارٍ إنشاء PDF…":"طباعة / PDF"}</button>
      </div>";
...
export default function TeacherNotesPage(){
  ...
  async function exportClassPdf(){
    ...
    setClassView(true);setPdfBusy(true);setMessage("");
    try{await new Promise(resolve=>setTimeout(resolve,100));const target=document.getElementById("nv13-class-report");if(!target)throw new Error();const html2canvas=(await import("html2canvas")).default;const{jsPDF}=await import("jspdf");const canvas=await html2canvas(target,{scale:1.8,backgroundColor:"#ffffff",useCORS:true});const pdf=new jsPDF({orientation:"portrait",unit:"mm",format:"a4"}),margin=7,w=196,h=canvas.height*w/canvas.width,pageH=283,data=canvas.toDataURL("image/png");for(let i=0;i<Math.max(1,Math.ceil(h/pageH));i++){if(i)pdf.addPage();pdf.addImage(data,"PNG",margin,margin-i*pageH,w,h);}pdf.save(reportScope==="all"?"ملاحظات-جميع-الفصول.pdf":`ملاحظات-${className}.pdf`);setMessage("تم إنشاء ملف PDF للملاحظات.");}catch{setMessage("تعذر إنشاء PDF الآن.");}finally{setPdfBusy(false);}
  }{classView?<section className="nv13-class-report" id="nv13-class-report"><header><div><small>سجل الملاحظات</small><h2>{reportScope==="all"?"جميع الفصول":className}</h2><p>{session?.subject||"المادة"} • المعلم: {session?.teacherName||"المعلم"}</p></div></header><div>{(reportScope==="all"?classes:[className]).map(reportClass=><section className="nv13-report-class" key={reportClass}><h2>{reportClass}</h2>{students.filter(student=>student.className===reportClass).map(student=>{const notes=[...(rows.find(row=>row.studentCode===student.code)?.notes||[])].sort((x,y)=>noteTime(y)-noteTime(x));return notes.length?<article key={student.code}><h3>{student.name}<small>{student.code}</small></h3>{notes.map(note=><div key={note.id||`${note.createdAt}-${note.message}`}><span>{note.label||"ملاحظة"} • {arabicDate(note.createdAt)}</span><p>{note.message}</p></div>)}</article>:null})}</section>)}</div></section>:null}
