...
export default function GradePlanPage() {
  ...
  function loadActiveForEdit() {
    ...
    if (!window.confirm("سيتم فتح نسخة قابلة للتعديل مبنية على خطة هذه المادة. الخطة المعتمدة لن تتغير إلا بعد اعتماد النسخة الجديدة. متابعة؟")) return;
    ...
    setMessage("تم السماح بالتعديل لهذه المادة. لن تتأثر الخطة المعتمدة حتى تضغط «اعتماد النسخة الجديدة».");
    setEditing(true);
  }
  ...
  function chooseSuggestion(nextMode: GradePlanMode) {
    ...
    setMessage(`تم تجهيز مقترح ${GRADE_PLAN_MODE_LABELS[nextMode]} لهذه المادة. راجعه وعدّله قبل الاعتماد.`);
  }
  ...
  async function approvePlan() {
    ...
    if (!subjectId) return setMessage("حدد المادة أولًا قبل اعتماد الخطة.");
    if (!checked.valid) return setMessage(checked.errors[0] || "أكمل توزيع الدرجات أولًا.");
    if (!window.confirm(`اعتماد هذه النسخة كخطة درجات ${session.subject || "المادة الحالية"}؟ سيتم الاحتفاظ بالنسخ السابقة في سجل المادة.`)) return;
    setSaving(true); setMessage("");
    try {
      const response = await fetch("/api/teacher/grade-plan", { method:"POST",headers:{"Content-Type":"application/json"},credentials:"same-origin",body:JSON.stringify({ subjectId, plan:checked.draft }) });
      const data = await response.json().catch(() => ({}));
      if (response.ok) {
        const serverPlan = normalizeGradePlan(data.activePlan);
        if (serverPlan) saveLocalGradePlan(serverPlan, subjectId); else if (session.teacherId) saveLocalGradePlan(createLocalGradePlan(checked.draft,session.teacherId,Number(data.version || activePlan?.version || 1)), subjectId);
        setMessage(`تم اعتماد خطة ${session.subject || "المادة"} الجديدة بنجاح.`); setEditing(false); window.setTimeout(() => window.location.reload(),350); return;
      }
      if (data.code === "grade_plan_quota_exceeded" && session.teacherId) {
        saveLocalGradePlan(createLocalGradePlan(checked.draft,session.teacherId,(activePlan?.version || 0)+1), subjectId);
        setMessage(`تم اعتماد خطة ${session.subject || "المادة"} على هذا الجهاز.`); setEditing(false); return;
      }
  ...
  if (loading) return <main className="gradeplan-v9"><div className="gpv9-loading">جارٍ قراءة خطة المادة…</div></main>;

  if (!editing) {
    return <main className="gradeplan-v9" dir="rtl">
      <section className="gpv9-hero"><div><small>الخطة الدراسية • {session.subject || "المادة الحالية"}</small><h1>هيكلة الدرجات الخاصة بالمادة</h1><p>كل مادة لها خطة رصد مستقلة من 100 درجة. لا يبدأ التعديل إلا بعد سماحك، والمقترحات لا تظهر إلا إذا طلبتها.</p></div><span>{activePlan ? "خطة معتمدة للمادة" : "لم تعتمد خطة للمادة بعد"}</span></section>
      ...
      {activePlan ? <>
        <section className="gpv9-current">
          <header><div><small>الخطة الحالية • {session.subject || "المادة"}</small><h2>{planLabel(activePlan.mode)}</h2><p>نسخة {activePlan.version} • {activePlan.method === "automatic" ? "بدأت باقتراح آلي ثم اعتمدها المعلم" : "إعداد يدوي"}</p></div><strong>100 درجة</strong></header>
        ...
        <section className="gpv9-permission"><div><span>🔒</span><h2>التعديل مقفل لهذه المادة</h2><p>هذا يمنع تغيير هيكلة درجات المادة بالخطأ أثناء الرصد. إذا كنت تريد نسخة جديدة افتح التعديل بإذن صريح.</p></div><div><button className="primary" type="button" onClick={loadActiveForEdit}>السماح بالتعديل</button><button type="button" onClick={() => setSuggestionsOpen(value => !value)}>{suggestionsOpen ? "إخفاء المقترحات" : "أحتاج مقترحات"}</button></div></section>
      </> : <section className="gpv9-empty"><span>📘</span><h2>ابدأ بخطة مناسبة لهذه المادة</h2><p>يمكنك إنشاء خطة المادة بنفسك أو طلب اقتراح جاهز ثم تعديله قبل الاعتماد.</p><div><button className="primary" type="button" onClick={loadActiveForEdit}>إعداد خطة المادة</button><button type="button" onClick={() => setSuggestionsOpen(value => !value)}>عرض المقترحات</button></div></section>}
      ...
      {history.length > 1 ? <details className="gpv9-history"><summary>عرض نسخ هذه المادة السابقة ({history.length})</summary><div>{history.map(plan => <span key={plan.id}><b>نسخة {plan.version}</b><small>{planLabel(plan.mode)}</small></span>)}</div></details> : null}
    </main>;
  }

  return <main className="gradeplan-v9" dir="rtl">
    <section className="gpv9-edit-head"><div><small>وضع التعديل • {session.subject || "المادة"}</small><h1>{activePlan ? "نسخة جديدة من خطة المادة" : "إعداد خطة المادة الأولى"}</h1><p>أي تغيير هنا مسودة فقط حتى الاعتماد.</p></div><button type="button" onClick={() => { setEditing(false); setMessage(""); }}>إلغاء والعودة للعرض</button></section>
    ...
    <section className={`gpv9-validation ${validation.valid ? "valid" : "invalid"}`}><div><small>التحقق النهائي</small><h2>{validation.valid ? "خطة المادة جاهزة" : "راجع التوزيع"}</h2><p>{draft.mode === "periods" ? `الفترة الأولى ${overall[0]||0}/100 • الفترة الثانية ${overall[1]||0}/100` : `المجموع ${overall[0]||0}/100`}</p>{validation.errors.slice(0,4).map(item => <span key={item}>• {item}</span>)}</div><button type="button" disabled={!validation.valid || saving || !subjectId} onClick={() => void approvePlan()}>{saving ? "جارٍ الاعتماد..." : "اعتماد خطة المادة"}</button></section>
  </main>;
}
