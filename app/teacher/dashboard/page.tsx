...
type SubjectAchievement={subjectId:string;hasPlan:boolean;planSource:string;planMode:string;planVersion:number;maximum:number;students:number;gradedStudents:number;achievement:number;originalAchievement:number;completion:number;deductedStudents:number;deductionTotal:number;averageDeduction:number};
...
export default function TeacherDashboardPage(){
  ...
  const [subjectAchievements,setSubjectAchievements]=useState<Record<string,SubjectAchievement>>({});
  ...
  const subjectSignature=subjectWorkspaces.map(subject=>baseSubject(subject.subjectId)).join("|");
  ...
  useEffect(()=>{
    ...
    const controller=new AbortController();const params=new URLSearchParams({subjectId});if(session.activeGrade)params.set("grade",String(session.activeGrade));
    Promise.all([
      fetch(`/api/teacher/students?${params}`,{cache:"no-store",signal:controller.signal}).then(async response=>{const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.message||"تعذر تحميل فصولك");return data;}),
    ...
    fetch(`/api/teacher/timetable?subjectId=${encodeURIComponent(subjectId)}`,{cache:"no-store",signal:controller.signal}).then(response=>response.ok?response.json():Promise.reject()).then(data=>setTimetable(data.lessons&&typeof data.lessons==="object"?data.lessons:{})).catch(()=>setTimetable({}));
    return()=>{controller.abort();stopAttendance();};
  },[session?.teacherId,session?.subjectKey,session?.activeGrade]);

  useEffect(()=>{
    if(!session?.teacherId||!subjectSignature)return;
    const controller=new AbortController();
    fetch("/api/teacher/subject-achievement",{cache:"no-store",signal:controller.signal}).then(async response=>{const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.message||"تعذر تحميل تحصيل المواد");return data;}).then(data=>{const map:Record<string,SubjectAchievement>={};(Array.isArray(data.summaries)?data.summaries:[]).forEach((item:SubjectAchievement)=>{if(item?.subjectId)map[item.subjectId]=item;});setSubjectAchievements(map);}).catch(()=>{});
    return()=>controller.abort();
  },[session?.teacherId,subjectSignature]);
  ...
  const studentStats=useMemo(()=>students.map(student=>{const result=activePlan?calculateGradePlanResult(activePlan,student):null;const deduction=result&&activePlan?deductionTotal(student,activePlan.id):0;const adjusted=result?Math.max(0,result.earned-deduction):0;return{...student,average:result?Math.round(adjusted):0,completion:result?.completion||0,hasGrade:Boolean(result&&result.recordedMaximum>0),deduction};}),[students,activePlan]);
  ...
  const aiInsight=supportStudents.length
    ? {title:`${supportStudents.length} طالب يحتاجون دعمًا`,copy:`ابدأ بـ ${supportStudents[0].name} في ${supportStudents[0].class}. تحصيله الحالي بعد الخصم ${supportStudents[0].average} من 100.`,href:"/teacher/follow-up",action:"فتح خطة الدعم"}
    : incompleteClasses.length
      ...
      : overall
        ? {title:"الأعمال الأساسية مستقرة",copy:`متوسط التحصيل الحالي بعد الخصومات ${overall} من 100. الآن أفضل وقت لمقارنة الفصول أو مراجعة المتميزين والمتعثرين.`,href:"/teacher/report",action:"مقارنة الفصول"}
        : {title:"جهّز بياناتك الأساسية",copy:"أضف جدولك واعتمد خطة المادة؛ بعدها تتحول الصفحة إلى مساعد يومي مبني على بياناتك الفعلية.",href:"/teacher/grade-plan",action:"فتح خطة المادة"};
  ...
  return <main className="teacher-dashboard-v16" dir="rtl">
    ...
    <section className="td16-kpis">
      ...
      <article data-tone="blue"><span>متوسط التحصيل</span><b>{graded.length?`${overall} / 100`:"— / 100"}</b><small>{graded.length} طالب لديهم رصد • بعد الخصم</small><i style={{width:`${overall}%`}}/></article>
    ...
    <section className="td16-main-grid">
      <article className="td16-subjects-panel">
        <header><div><small>المواد المسندة لي</small><h2>خطط المواد والتحصيل من 100</h2></div><span>{subjectWorkspaces.length} مادة</span></header>
        <div className="td16-subject-cards">{subjectWorkspaces.map(subject=>{const active=subject.workspaceKey===session.workspaceKey;const subjectId=baseSubject(subject.subjectId);const summary=subjectAchievements[subjectId];const score=summary?.gradedStudents?summary.achievement:active&&graded.length?overall:null;const deduction=summary?.deductionTotal||0;const deductedStudents=summary?.deductedStudents||0;return <button type="button" key={subject.workspaceKey} data-subject={subject.subjectId} className={active?"active":""} onClick={()=>{if(!active)void session.setSubject?.(subject.workspaceKey);}}><span className="td16-subject-art" data-subject={subject.subjectId}/><div><small>{subject.gradeLabel||"مساحة تعليمية"}</small><h3>{subject.subjectName}</h3><p>{subjectTagline(subject.subjectId)}</p><span className={`td16-plan-state ${summary?.hasPlan?"ready":"missing"}`}>{summary?.hasPlan?`خطة الرصد • نسخة ${summary.planVersion}`:"لم تعتمد خطة مستقلة للمادة"}</span></div><footer className="td16-achievement-footer"><b>{score==null?"— / 100":`${score} / 100`}</b><span>التحصيل العلمي</span>{deduction>0?<em>خصم −{deduction} • {deductedStudents} طالب</em>:summary?.hasPlan?<em className="clear">بدون خصم نشط</em>:<em>افتح المادة واعتمد خطتها</em>}</footer></button>;})}</div>
      </article>
