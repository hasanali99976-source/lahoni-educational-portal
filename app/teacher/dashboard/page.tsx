...
export default function TeacherDashboardPage() {
  ...
  return <main className="analytics-dashboard" dir="rtl">
    <section className="analytics-heading"><div><span>التحليل الذكي لأداء الطلاب</span><h1>لوحة تحليل {subject}</h1><p>اختر جميع الطلاب أو فصلًا أو طالبًا واحدًا لعرض مقارنة دقيقة بين عناصر الأداء.</p></div><div className="analysis-scope" role="group" aria-label="نطاق التحليل"><button className={scope === "all" ? "active" : ""} onClick={() => changeScope("all")}>جميع الطلاب</button><button className={scope === "class" ? "active" : ""} onClick={() => changeScope("class")}>فصل محدد</button><button className={scope === "student" ? "active" : ""} onClick={() => changeScope("student")}>طالب واحد</button></div></section>
    <section className="analytics-filters">
      ...
      <div className="analysis-current"><small>التحليل الحالي</small><strong>{scope === "all" ? "جميع الطلاب" : scope === "class" ? selectedClass || "اختر فصلًا" : selectedAnalysis?.name || "اختر طالبًا"}</strong></div>
    </section>
