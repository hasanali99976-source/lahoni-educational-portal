...
function missingCount(student:Student){
  ...
  unitKeys.forEach(key=>{
    const r=student.units?.[key]||{};
    if(r.attendance===undefined) count++;
    if(r.participation===undefined) count++;
    if(r.homework===undefined) count++;
    if(r.unitExam===undefined) count++;
  });
...
export default function FollowUpPage(){
  ...
  async function saveNote(){
    ...
    await updateDoc(doc(db,"students",noteStudent.id),{teacherNote:note.trim()});
    setSaved("تم حفظ ملاحظة المعلم وتظهر الآن لولي الأمر");
    setTimeout(()=>setSaved(""),2500);
  }

  return <main className="follow-page" dir="rtl">
    <section className="follow-head">
      <div><span>متابعة التحصيل الدراسي</span><h1>الإتقان والتنبيهات الذكية</h1><p>قائمة فورية للطلاب الأقل من معيار الإتقان مع أدوات جاهزة للموجّه الطلابي.</p></div>
      <div className="follow-filters">
        <label>الفصل<select value={selectedClass} onChange={e=>setSelectedClass(e.target.value)}><option value="">جميع الفصول</option>{classes.map(c=><option key={c}>{c}</option>)}</select></label>
        <label>معيار الإتقان<select value={threshold} onChange={e=>setThreshold(Number(e.target.value))}><option value={80}>أقل من ٨٠٪</option><option value={75}>أقل من ٧٥٪</option><option value={70}>أقل من ٧٠٪</option></select></label>
      </div>
    </section>

    <section className="follow-overview">
      <article><span>عدد الطلاب</span><strong>{ranked.length}</strong></article>
      <article><span>متوسط الفصل</span><strong>{average}%</strong></article>
      <article className="warn"><span>غير المتقنين</span><strong>{nonMastered.length}</strong></article>
      <article className="alert"><span>ناقصو الرصد</span><strong>{incomplete.length}</strong></article>
    </section>

    <section className="follow-grid">
      <article className="follow-card top-card"><header><div><h2>أفضل الطلاب</h2><p>الترتيب حسب المجموع النهائي.</p></div></header><div className="top-students">{ranked.slice(0,3).map((s,i)=><div key={s.id}><b>{["🥇","🥈","🥉"][i]}</b><span>{s.name}</span><strong>{s.total}</strong><small>{s.total-average>=0?`أعلى من المتوسط بـ ${s.total-average}`:`أقل من المتوسط بـ ${average-s.total}`}</small></div>)}</div></article>
      <article className="follow-card alerts-card"><header><div><h2>تنبيهات الرصد</h2><p>طلاب لديهم خانات لم تُرصد بعد.</p></div></header><div className="alerts-list">{incomplete.slice(0,8).map(s=><div key={s.id}><span>{s.name}</span><b>{s.missing} خانة ناقصة</b></div>)}{!incomplete.length&&<p className="empty">جميع الدرجات مكتملة.</p>}</div></article>
    </section>
