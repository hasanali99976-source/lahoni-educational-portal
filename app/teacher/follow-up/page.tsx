...
export default function FollowUpPage() {
  ...
  function printMasteryTable() {
    ...
    const targetClasses=printScope==="all"?classes:(selectedClass?[selectedClass]:(masteryGroups[0]?.className?[masteryGroups[0].className]:[]));
    ...
    win.document.write(`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>تقرير الإتقان</title><style>@page{size:A4 portrait;margin:8mm}*{box-sizing:border-box}body{margin:0;font-family:Tahoma,Arial,sans-serif;color:#17324d}.sheet{min-height:0;position:relative;padding-bottom:8mm;break-inside:avoid;page-break-inside:avoid}.new-page{break-before:page;page-break-before:always}header{display:flex;align-items:center;justify-content:space-between;border:1px solid #bdd0e2;border-top:5px solid #1768c5;padding:6px 8px}header small{font-size:8px;color:#607d98}header h1{margin:2px 0 0;color:#124d87;font-size:16px}header>strong{background:#edf5fd;color:#155da8;border:1px solid #c9ddf0;padding:5px 8px;border-radius:6px}.meta{display:grid;grid-template-columns:repeat(4,1fr);gap:5px;margin:6px 0}.meta span{display:flex;flex-direction:column;gap:2px;border:1px solid #d8e4ef;background:#f8fbfe;padding:4px 6px;font-size:7px;color:#71869a}.meta b{font-size:10px;color:#183f64}table{width:100%;border-collapse:collapse;table-layout:fixed;font-size:7.5px}th,td{border:1px solid #cad8e5;padding:2.5px 3px;text-align:center;line-height:1.05}th{background:#1768c5;color:#fff}th:nth-child(1){width:7%}th:nth-child(2){width:39%}th:nth-child(3){width:13%}th:nth-child(4){width:24%}th:nth-child(5){width:17%}.name{text-align:right;font-weight:700}tbody tr:nth-child(even){background:#f6f9fc}.empty{padding:18px;color:#6d8295}footer{margin-top:5px;display:flex;justify-content:space-between;border-top:1px solid #d4e0ea;padding-top:6px;font-size:8px;color:#708397}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body>${pages}</body></html>`);win.document.close();setTimeout(()=>win.print(),250);
  }
  ...
  return <main className="mastery-v140" dir="rtl">
