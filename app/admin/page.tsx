...
export default function AdminPage() {
  ...
  if (!authenticated) return <main className="admin-login" dir="rtl"><section><Link href="/">العودة للرئيسية</Link><span>إدارة البوابة</span><h1>دخول مدير النظام</h1><p>من هنا تنشئ حسابات المعلمين وتحدد المواد والصلاحيات.</p><form onSubmit={login}><label>اسم المستخدم<input value={username} onChange={(event) => setUsername(event.target.value)} required /></label><label>كلمة المرور<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} required /></label>{message && <p className="admin-message error">{message}</p>}<button disabled={busy}>{busy ? "جارٍ الدخول…" : "دخول لوحة الإدارة"}</button></form></section></main>;
