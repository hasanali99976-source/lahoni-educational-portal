...
export default function BulkRosterImport() {
  ...
  function selectFile(next: File | null) { setFile(next); setRows([]); setMessage(""); if (next) setExpanded(true); }
  function onDrop(event: DragEvent<HTMLDivElement>) { event.preventDefault(); setDragging(false); const next = event.dataTransfer.files?.[0] || null; if (next) selectFile(next); }
  ...
  return <section className={`smart-roster-import-shell ${expanded ? "is-expanded" : "is-collapsed"}`} dir="rtl">
    {!expanded ? <button type="button" className="roster-compact-launcher" onClick={() => setExpanded(true)}><span>⇧</span><div><strong>رفع قائمة طلاب</strong><small>Excel / CSV / PDF عند الحاجة فقط</small></div><b>فتح أداة الرفع</b></button> : <div className="smart-roster-import">
      <header>
        <div className="smart-roster-import-title"><span>⇧</span><div><small>إضافة جماعية عند الحاجة</small><h1>رفع قائمة الطلاب</h1><p>اختر الملف ثم راجع الأسماء قبل اعتمادها.</p></div></div>
        <div className="roster-import-head-actions"><button type="button" className="roster-advanced-toggle" onClick={toggleAdvanced}>{showAdvanced ? "إخفاء الأدوات اليدوية" : "إدارة متقدمة"}</button><button type="button" className="roster-collapse-button" onClick={() => setExpanded(false)}>تصغير</button></div>
      </header>
