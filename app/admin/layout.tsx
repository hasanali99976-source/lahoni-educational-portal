...
const sections = [{ href: "/admin", label: "الرئيسية", type: "home" as const },{ href: "/admin/teachers", label: "إدارة المعلمين", type: "teachers" as const },{ href: "/admin/students", label: "إدارة الطلاب", type: "students" as const },{ href: "/admin/competition", label: "التنافس بين المعلمين", type: "competition" as const }];
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  const pathname = usePathname(); const [authenticated,setAuthenticated]=useState<boolean|null>(null);
  useEffect(()=>{let live=true;setAuthenticated(null);fetch("/api/auth/admin-session",{cache:"no-store",credentials:"same-origin"}).then(r=>{if(live)setAuthenticated(r.ok)}).catch(()=>{if(live)setAuthenticated(false)});return()=>{live=false}},[pathname]);
  ...
  async function logout(){await fetch("/api/auth/logout",{method:"POST",cache:"no-store"}).catch(()=>undefined);window.location.assign("/admin")}
  return <div className="admin-v4-frame portal-v100" dir="rtl"><div className="admin-main-scene" aria-hidden="true"/><aside className="admin-v4-sidebar"><div className="admin-v4-sidebrand"><Image src="/icons/lahooni-identity-320.jpg" alt="هوية بوابة أستاذ لحوني التعليمية" width={64} height={64} priority/><div><small>بوابة الإدارة</small><strong>أستاذ لحوني التعليمية</strong></div></div><nav className="admin-v4-side-nav" aria-label="أقسام الإدارة">{sections.map(item=>{const active=item.href==="/admin"?pathname==="/admin":pathname.startsWith(item.href);return <Link key={item.href} href={item.href} className={active?"active":""}><AdminIcon type={item.type}/><span>{item.label}</span></Link>})}</nav><div className="admin-v4-sidequote"><span>مركز الإدارة المدرسية</span><i/></div><div className="admin-v4-side-signature">تصميم وتنفيذ<br/><b>الأستاذ حسن علي الطويل</b></div></aside><section className="admin-v4-main"><section className="admin-v4-identity"><div className="admin-v4-identity-copy"><small>مركز الإدارة المدرسية الذكي</small><h1>{title}</h1><p>{description}</p></div><div className="admin-v4-person"><Image src="/teacher/teacher-avatar.svg" alt="أيقونة المعلم" width={64} height={64}/><div><strong>الأستاذ حسن علي الطويل</strong><small>مدير بوابة أستاذ لحوني التعليمية</small></div><button type="button" onClick={logout}>تسجيل الخروج</button></div></section><main className="admin-v4-workspace">{children}</main></section></div>;
}
