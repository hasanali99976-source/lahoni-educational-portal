...
test("يعرض خطأ الدخول بدون كشف تفاصيل الحساب", async ({ page }) => {
  ...
  await page.getByLabel("اسم المعلم").fill("unknown");
  await page.getByLabel("الرقم السري").fill("wrong-password");
  await page.getByRole("button", { name: /دخول بوابة المعلم/ }).click();
