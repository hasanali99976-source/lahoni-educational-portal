...
import_line = 'import { downloadDiagnosticPdfDocument } from "../../../lib/diagnostic-pdf";\n'
anchor = 'import DiagnosticResults from "./diagnostic-results";\n'
if import_line not in text:
    text = text.replace(anchor, anchor + import_line)

helper_anchor = '  function printPreviewTest(item: Diagnostic) {'
helper = '''  async function downloadDiagnosticPdf(item: Diagnostic, teacherCopy = false) {\n    setMessage(teacherCopy ? "جارٍ تجهيز PDF كامل بالإجابات…" : "جارٍ تجهيز PDF كامل للطالب…");\n    try {\n      const result = await downloadDiagnosticPdfDocument({\n        portalName: PORTAL_NAME,\n        teacherName: session?.teacherName || "المعلم",\n        subject: session?.subject || "المادة",\n        gradeLabel: session?.activeGradeLabel || "",\n        title: item.title,\n        instructions: item.instructions,\n        questions: item.questions,\n        teacherCopy,\n        fileName: `${item.title.replace(/[\\\\/:*?\"<>|]/g, "-")}-${teacherCopy ? "نسخة-المعلم" : "نسخة-الطالب"}.pdf`,\n      });\n      setMessage(`تم إنشاء PDF كامل: ${result.questionCount} سؤالًا في ${result.pageCount} صفحة.`);\n    } catch (error) {\n      console.error("diagnostic-pdf-v100", error);\n      setMessage("تعذر إنشاء PDF الاختبار الآن.");\n    }\n  }\n\n'''
if helper not in text:
    text = text.replace(helper_anchor, helper + helper_anchor)

old_footer = '<footer><button type="button" onClick={() => printPreviewTest(preview)}>طباعة الاختبار</button><button type="button" className="primary" onClick={() => setPreview(null)}>إغلاق</button></footer>'
new_footer = '<footer><button type="button" onClick={() => void downloadDiagnosticPdf(preview, false)}>📄 PDF نسخة الطالب كاملة</button><button type="button" onClick={() => void downloadDiagnosticPdf(preview, true)}>✅ PDF نسخة المعلم بالإجابات</button><button type="button" className="primary" onClick={() => setPreview(null)}>إغلاق</button></footer>'
if old_footer in text:
    text = text.replace(old_footer, new_footer)
elif new_footer not in text:
    raise SystemExit('diagnostic preview footer anchor not found')
...
'''
Path('lib/diagnostic-pdf.ts').write_text(pdf, encoding='utf-8')

pwa_path = Path('app/pwa-register.tsx')
pwa = pwa_path.read_text(encoding='utf-8')
pwa = pwa.replace('ostadh-lahooni-v98-full-grades-pdf', 'ostadh-lahooni-v100-diagnostic-full-pdf')
pwa = pwa.replace('/sw.js?v=98-full-grades-pdf', '/sw.js?v=100-diagnostic-full-pdf')
pwa = pwa.replace('ostadh-lahooni-v99-gradebook-student-sync', 'ostadh-lahooni-v100-diagnostic-full-pdf')
pwa = pwa.replace('/sw.js?v=99-gradebook-student-sync', '/sw.js?v=100-diagnostic-full-pdf')
pwa_path.write_text(pwa, encoding='utf-8')

sw_path = Path('public/sw.js')
sw = sw_path.read_text(encoding='utf-8')
import re
sw = re.sub(r'const CACHE_NAME = "[^"]+";', 'const CACHE_NAME = "ostadh-lahooni-v100-diagnostic-full-pdf";', sw, count=1)
